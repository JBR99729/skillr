---
curriculum_code: AC9M5N05
subject: Mathematics
year_level: Year 5
description: solve problems involving addition and subtraction of fractions with the same or related denominators, using different strategies
batch: 1
practice_questions: 68
exam_questions: 30
worksheet_questions: 10
worksheet_practice_selection: 8
worksheet_exam_selection: 2
launch_practice_ids: P01,P18,P35,P52,P09,P26,P43,P60
quick_read_status: authored
review_status: authored
---

# AC9M5N05 — Year 5 Mathematics digital question bank

## 60-second Quick Read

- **Key idea:** This Year 5 skill is about solve problems involving addition and subtraction of fractions with the same or related denominators, using different strategies.
- **Method:** Read the question carefully, identify the values or relationships, choose the most efficient strategy, then check whether the answer makes sense in the context.
- **Worked example:** For a screen task, first match each number, model or condition to its mathematical meaning. Then use the tool or answer box only after you have compared the key place values, units, operations or features.
- **Digital model:** Online questions may use movable cards, selectable diagrams, number-line widgets, grid tools, tables, graphs or constrained answer boxes. Units should sit outside numeric input boxes.
- **Common trap:** Do not answer from the surface pattern only. Check the operation, unit, direction, scale, denominator, coordinate order or likelihood before selecting your final answer.

## Practice questions

## Tier 1 — Knowledge and terms

### P01 — Solve 1 4 2 4 and use

**Type:** Multiple choice

**Delivery:** Show the required on-screen visual, model or widget; include an accessible text alternative. Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Solve 1/4 + 2/4 and use the on-screen tool to build or select a model.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing solve tool build model.

### P02 — Solve 1 2 1 4 by rewriting

**Type:** Short response

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Solve 1/2 + 1/4 by rewriting one fraction with an equivalent denominator.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing solve rewriting fraction equivalent denominator.

### P03 — A recipe needs 3 4 cup flour

**Type:** Short response

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** A recipe needs 3/4 cup flour and 1/4 cup oats. How much dry ingredient is this altogether?

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing recipe needs flour oats much ingredient this altogether.

### P04 — Subtract 2 3 1 6 Show your

**Type:** Short response

**Delivery:** Show the required on-screen visual, model or widget; include an accessible text alternative. Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Subtract 2/3 - 1/6. Show your working on a number line.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing subtract show your working number line.

### P05 — A walk is 2 1 4 km

**Type:** Short response

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** A walk is 2 1/4 km. Mia has walked 3/4 km. How far remains?

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing walk walked remains.

### P06 — Solve 5 8 1 4 Explain why

**Type:** Multiple choice

**Question:** Solve 5/8 + 1/4. Explain why 1/4 can be renamed.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing solve renamed.

### P07 — Find the missing fraction 1 3 5

**Type:** Short response

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Find the missing fraction: 1/3 + ___ = 5/6.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing find missing fraction.

### P08 — A clock shows half an hour plus

**Type:** Short response

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** A clock shows half an hour plus three-quarters of an hour. How many hours is that?

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing clock shows half hour plus three quarters hour.

### P09 — Solve 1 1 2 2 4 Give

**Type:** Short response

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Solve 1 1/2 - 2/4. Give the answer as a mixed numeral.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing solve mixed numeral.

### P10 — True or false 1 5 1 10

**Type:** Multiple choice

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** True or false: 1/5 + 1/10 = 2/15. Explain the mistake.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing true false mistake.

### P11 — Use a bar model to solve 3

**Type:** Short response

**Delivery:** Show the required on-screen visual, model or widget; include an accessible text alternative. Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Use a bar model to solve 3/6 + 1/3.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing model solve.

### P12 — A rope is 1 3 4 m

**Type:** Short response

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** A rope is 1 3/4 m long. 1/2 m is cut off. How much is left?

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing rope long much left.

### P13 — Solve 7 10 2 5 Show equivalent

**Type:** Short response

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Solve 7/10 - 2/5. Show equivalent fractions.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing solve show equivalent fractions.

### P14 — Complete the structured template for a fraction

**Type:** Short response

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Complete the structured template for a fraction addition problem with answer 1 whole.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing fraction addition problem whole.

### P15 — Solve 2 8 3 4 Explain your

**Type:** Multiple choice

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Solve 2/8 + 3/4. Explain your strategy.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing solve your strategy.

### P16 — A jug has 5 6 L of

**Type:** Short response

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** A jug has 5/6 L of water. 1/3 L is poured out. How much remains?

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing water poured much remains.

### P17 — Which is larger 1 2 1 8

**Type:** Multiple choice

**Question:** Which is larger: 1/2 + 1/8 or 3/4 - 1/8? Explain.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing larger.

## Tier 2 — Core skill practice

### P18 — Solve 1 2 3 1 6 Show

**Type:** Short response

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Solve 1 2/3 + 1/6. Show how the fractional parts combine.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing solve show fractional parts combine.

### P19 — Find two different fraction sums equal to

**Type:** Short response

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Find two different fraction sums equal to 3/4.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing find different fraction sums equal.

### P20 — A student adds denominators in 2 5

**Type:** Short response

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** A student adds denominators in 2/5 + 1/5 to get 3/10. Correct the work.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing adds denominators correct work.

### P21 — Use a number line to solve 1

**Type:** Short response

**Delivery:** Show the required on-screen visual, model or widget; include an accessible text alternative. Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Use a number line to solve 1/4 + 1/4 + 1/2.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing number line solve.

### P22 — Solve 2 1 5 3 5 Explain

**Type:** Multiple choice

**Question:** Solve 2 1/5 - 3/5. Explain any regrouping.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing solve regrouping.

### P23 — Choose the matching story problem for 5

**Type:** Matching

**Delivery:** Online, provide movable cards with drag, tap-to-place and keyboard controls. Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Choose the matching story problem for 5/6 - 1/2 and solve it.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Each card must be matched to the representation with the same value or meaning.

**Coverage:** Online Year 5 task assessing matching story problem solve.

### P24 — Explain when denominators need to be made

**Type:** Multiple choice

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Explain when denominators need to be made the same before adding or subtracting.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing when denominators need made same before adding subtracting.

### P25 — Solve 3 8 5 16 and explain

**Type:** Multiple choice

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Solve 3/8 + 5/16 and explain why sixteenths help.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing solve sixteenths help.

### P26 — A runner completes 5 6 km rests

**Type:** Short response

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** A runner completes 5/6 km, rests, then completes 7/12 km. How far did they run altogether?

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing runner completes rests completes they altogether.

### P27 — Calculate 2 1 4 5 8 using

**Type:** Short response

**Delivery:** Show the required on-screen visual, model or widget; include an accessible text alternative. Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Calculate 2 1/4 - 5/8 using the interactive number line or equivalent fractions.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing calculate interactive number line equivalent fractions.

### P28 — A student adds 1 3 1 6

**Type:** Multiple choice

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** A student adds 1/3 + 1/6 and gets 2/9. Diagnose the mistake and solve correctly.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing adds gets diagnose mistake solve correctly.

### P29 — Find the missing fraction 7 10 1

**Type:** Short response

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Find the missing fraction: 7/10 + ___ = 1 1/5.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing find missing fraction.

### P30 — Solve 3 2 3 1 5 6

**Type:** Short response

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Solve 3 2/3 - 1 5/6. Show regrouping if needed.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing solve show regrouping needed.

### P31 — Which fraction strip model correctly represents 1

**Type:** Multiple choice

**Delivery:** Show the required on-screen visual, model or widget; include an accessible text alternative. Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Which fraction-strip model correctly represents 1/2 + 3/8 + 1/4?

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing fraction strip model correctly represents.

### P32 — A bottle is 7 8 full You

**Type:** Short response

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** A bottle is 7/8 full. You pour out 1/4. What fraction remains?

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing bottle full pour fraction remains.

### P33 — Order these sums from smallest to largest

**Type:** Sequencing

**Delivery:** Online, provide movable cards with drag, tap-to-place and keyboard controls. Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Order these sums from smallest to largest without fully calculating all first: 1/2+1/8, 2/3+1/6, 3/4+1/12.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. The order must be complete and use the direction requested in the prompt.

**Coverage:** Online Year 5 task assessing order sums smallest largest without fully calculating first.

### P34 — Solve 5 12 1 6 3 4

**Type:** Short response

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Solve 5/12 + 1/6 + 3/4 and write the answer as a mixed numeral if needed.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing solve write mixed numeral needed.

## Tier 3 — Guided application

### P35 — A recipe needs 1 1 2 cups

**Type:** Short response

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** A recipe needs 1 1/2 cups oats and 3/4 cup coconut. How much more oats than coconut?

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing recipe needs cups oats coconut much more oats.

### P36 — Find two different fraction pairs with related

**Type:** Short response

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Find two different fraction pairs with related denominators that add to 1 1/4.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing find different fraction pairs related denominators.

### P37 — Explain when you can add numerators directly

**Type:** Multiple choice

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Explain when you can add numerators directly and when you must rename fractions first.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing when numerators directly when must rename fractions first.

### P38 — Solve 2 7 10 using tenths then

**Type:** Multiple choice

**Question:** Solve 2 - 7/10 using tenths, then explain the whole-number regrouping.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing solve tenths whole number regrouping.

### P39 — A garden has 3 5 planted with

**Type:** Short response

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** A garden has 3/5 planted with vegetables and 1/4 with flowers. What fraction is planted? What fraction is unplanted?

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing garden planted vegetables flowers fraction planted fraction unplanted.

### P40 — Calculate 11 12 2 3 Then choose

**Type:** Multiple choice

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Calculate 11/12 - 2/3. Then choose the diagram that proves the subtraction correctly.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing calculate diagram proves subtraction correctly.

### P41 — Write a word problem represented by 1

**Type:** Short response

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Write a word problem represented by 1 3/8 + 7/8. Solve it.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing write word problem represented solve.

### P42 — Find the error 4 5 1 10

**Type:** Multiple choice

**Question:** Find the error: 4/5 - 1/10 = 3/5. Correct it.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing find error correct.

### P43 — Solve 2 5 6 1 3 4

**Type:** Multiple choice

**Question:** Solve 2 5/6 + 1 3/4. Explain how you handled the denominators.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing solve handled denominators.

### P44 — A trail is 4 km You walk

**Type:** Short response

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** A trail is 4 km. You walk 1 2/5 km then 1 3/10 km. How much remains?

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing trail walk much remains.

### P45 — Make a common denominator table for halves

**Type:** Short response

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Make a common denominator table for halves, quarters, eighths and sixteenths.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing make common denominator table halves quarters eighths sixteenths.

### P46 — Find the missing sign 3 4 1

**Type:** Multiple choice

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Find the missing sign: 3/4 + 1/8 ___ 5/6. Justify.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing find missing sign justify.

### P47 — Solve 1 1 3 5 12 then

**Type:** Multiple choice

**Question:** Solve 1 1/3 - 5/12, then estimate first to check reasonableness.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing solve estimate first check reasonableness.

### P48 — Complete the structured template for a multi

**Type:** Short response

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Complete the structured template for a multi-step fraction addition/subtraction problem with related denominators and solve it.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing multi step fraction addition subtraction problem related denominators.

### P49 — Beginner digital question Solve 1 4 2

**Type:** Multiple choice

**Delivery:** Show the required on-screen visual, model or widget; include an accessible text alternative. Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Beginner digital question: Solve 1/4 + 2/4 and use the on-screen tool to build or select a model.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing solve tool build model.

### P50 — Use the on screen options Solve 1

**Type:** Short response

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Use the on-screen options: Solve 1/2 + 1/4 by rewriting one fraction with an equivalent denominator.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing solve rewriting fraction equivalent denominator.

### P51 — Complete the answer box A recipe needs

**Type:** Short response

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Complete the answer box: A recipe needs 3/4 cup flour and 1/4 cup oats. How much dry ingredient is this altogether?

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing recipe needs flour oats much ingredient this altogether.

## Tier 4 — Problem solving and reasoning

### P52 — Match or choose Subtract 2 3 1

**Type:** Matching

**Delivery:** Online, provide movable cards with drag, tap-to-place and keyboard controls. Show the required on-screen visual, model or widget; include an accessible text alternative.

**Question:** Match or choose: Subtract 2/3 - 1/6. Show your working on a number line.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Each card must be matched to the representation with the same value or meaning.

**Coverage:** Online Year 5 task assessing match subtract show your working number line.

### P53 — Select the best answer A walk is

**Type:** Multiple choice

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Select the best answer: A walk is 2 1/4 km. Mia has walked 3/4 km. How far remains?

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing best walk walked remains.

### P54 — Beginner digital question Solve 5 8 1

**Type:** Multiple choice

**Question:** Beginner digital question: Solve 5/8 + 1/4. Explain why 1/4 can be renamed.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing solve renamed.

### P55 — Use the on screen options Find the

**Type:** Short response

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Use the on-screen options: Find the missing fraction: 1/3 + ___ = 5/6.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing find missing fraction.

### P56 — Complete the answer box A clock shows

**Type:** Short response

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Complete the answer box: A clock shows half an hour plus three-quarters of an hour. How many hours is that?

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing clock shows half hour plus three quarters hour.

### P57 — Match or choose Solve 1 1 2

**Type:** Matching

**Delivery:** Online, provide movable cards with drag, tap-to-place and keyboard controls. Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Match or choose: Solve 1 1/2 - 2/4. Give the answer as a mixed numeral.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Each card must be matched to the representation with the same value or meaning.

**Coverage:** Online Year 5 task assessing match solve mixed numeral.

### P58 — Select the best answer True or false

**Type:** Multiple choice

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Select the best answer: True or false: 1/5 + 1/10 = 2/15. Explain the mistake.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing best true false mistake.

### P59 — Beginner digital question Use a bar model

**Type:** Short response

**Delivery:** Show the required on-screen visual, model or widget; include an accessible text alternative. Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Beginner digital question: Use a bar model to solve 3/6 + 1/3.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing model solve.

### P60 — Use the on screen options A rope

**Type:** Short response

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Use the on-screen options: A rope is 1 3/4 m long. 1/2 m is cut off. How much is left?

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing rope long much left.

### P61 — Complete the answer box Solve 7 10

**Type:** Short response

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Complete the answer box: Solve 7/10 - 2/5. Show equivalent fractions.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing solve show equivalent fractions.

### P62 — Match or choose Complete the structured template

**Type:** Matching

**Delivery:** Online, provide movable cards with drag, tap-to-place and keyboard controls. Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Match or choose: Complete the structured template for a fraction addition problem with answer 1 whole.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Each card must be matched to the representation with the same value or meaning.

**Coverage:** Online Year 5 task assessing match fraction addition problem whole.

### P63 — Select the best answer Solve 2 8

**Type:** Multiple choice

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Select the best answer: Solve 2/8 + 3/4. Explain your strategy.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing best solve your strategy.

### P64 — Beginner digital question A jug has 5

**Type:** Short response

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Beginner digital question: A jug has 5/6 L of water. 1/3 L is poured out. How much remains?

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing water poured much remains.

### P65 — Use the on screen options Which is

**Type:** Multiple choice

**Question:** Use the on-screen options: Which is larger: 1/2 + 1/8 or 3/4 - 1/8? Explain.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing larger.

### P66 — Complete the answer box Solve 1 2

**Type:** Short response

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Complete the answer box: Solve 1 2/3 + 1/6. Show how the fractional parts combine.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing solve show fractional parts combine.

### P67 — Match or choose Find two different fraction

**Type:** Matching

**Delivery:** Online, provide movable cards with drag, tap-to-place and keyboard controls. Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Match or choose: Find two different fraction sums equal to 3/4.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Each card must be matched to the representation with the same value or meaning.

**Coverage:** Online Year 5 task assessing match find different fraction sums equal.

### P68 — Select the best answer A student adds

**Type:** Multiple choice

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Select the best answer: A student adds denominators in 2/5 + 1/5 to get 3/10. Correct the work.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing best adds denominators correct work.

## Exam questions

## Section A — Core knowledge

### E01 — Beginner digital question Use a number line

**Type:** Short response

**Delivery:** Show the required on-screen visual, model or widget; include an accessible text alternative. Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Beginner digital question: Use a number line to solve 1/4 + 1/4 + 1/2.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing number line solve.

### E02 — Use the on screen options Solve 2

**Type:** Multiple choice

**Question:** Use the on-screen options: Solve 2 1/5 - 3/5. Explain any regrouping.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing solve regrouping.

### E03 — Complete the answer box Choose the matching

**Type:** Matching

**Delivery:** Online, provide movable cards with drag, tap-to-place and keyboard controls. Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Complete the answer box: Choose the matching story problem for 5/6 - 1/2 and solve it.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Each card must be matched to the representation with the same value or meaning.

**Coverage:** Online Year 5 task assessing matching story problem solve.

### E04 — Match or choose Explain when denominators need

**Type:** Matching

**Delivery:** Online, provide movable cards with drag, tap-to-place and keyboard controls. Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Match or choose: Explain when denominators need to be made the same before adding or subtracting.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Each card must be matched to the representation with the same value or meaning.

**Coverage:** Online Year 5 task assessing match when denominators need made same before adding.

### E05 — Calculate 1 4 2 4

**Type:** Short response

**Question:** Calculate 1/4 + 2/4.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing calculate.

### E06 — Intermediate digital question Solve 3 8 5

**Type:** Multiple choice

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Intermediate digital question: Solve 3/8 + 5/16 and explain why sixteenths help. Check the common trap before answering.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing solve sixteenths help check common trap before answering.

### E07 — Multi step online task A runner completes

**Type:** Matching

**Delivery:** Online, provide movable cards with drag, tap-to-place and keyboard controls. Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Multi-step online task: A runner completes 5/6 km, rests, then completes 7/12 km. How far did they run altogether? Use two steps and choose the matching reasoning.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Each card must be matched to the representation with the same value or meaning.

**Coverage:** Online Year 5 task assessing multi step online task runner completes rests completes.

### E08 — Reasoning check Calculate 2 1 4 5

**Type:** Multiple choice

**Delivery:** Show the required on-screen visual, model or widget; include an accessible text alternative. Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Reasoning check: Calculate 2 1/4 - 5/8 using the interactive number line or equivalent fractions. Interpret the answer in the story context.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing reasoning check calculate interactive number line equivalent fractions.

### E09 — Model based question A student adds 1

**Type:** Multiple choice

**Delivery:** Online, provide movable cards with drag, tap-to-place and keyboard controls. Show the required on-screen visual, model or widget; include an accessible text alternative. Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Model-based question: A student adds 1/3 + 1/6 and gets 2/9. Diagnose the mistake and solve correctly. Choose the model or method that matches your answer.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing model based adds gets diagnose mistake solve correctly.

### E10 — Error check question Find the missing fraction

**Type:** Multiple choice

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Error-check question: Find the missing fraction: 7/10 + ___ = 1 1/5. Estimate first, then enter the exact answer.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing error check find missing fraction estimate first enter.

## Section B — Application and problem solving

### E11 — Intermediate digital question Solve 3 2 3

**Type:** Short response

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Intermediate digital question: Solve 3 2/3 - 1 5/6. Show regrouping if needed. Check the common trap before answering.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing solve show regrouping needed check common trap before.

### E12 — Multi step online task Which fraction strip

**Type:** Multiple choice

**Delivery:** Show the required on-screen visual, model or widget; include an accessible text alternative. Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Multi-step online task: Which fraction-strip model correctly represents 1/2 + 3/8 + 1/4?

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing multi step online task fraction strip model correctly.

### E13 — Reasoning check A bottle is 7 8

**Type:** Multiple choice

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Reasoning check: A bottle is 7/8 full. You pour out 1/4. What fraction remains? Interpret the answer in the story context.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing reasoning check bottle full pour fraction remains interpret.

### E14 — Model based question Order these sums from

**Type:** Sequencing

**Delivery:** Online, provide movable cards with drag, tap-to-place and keyboard controls. Show the required on-screen visual, model or widget; include an accessible text alternative. Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Model-based question: Order these sums from smallest to largest without fully calculating all first: 1/2+1/8, 2/3+1/6, 3/4+1/12. Choose the model or method that matches your answer.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. The order must be complete and use the direction requested in the prompt.

**Coverage:** Online Year 5 task assessing model based order sums smallest largest without fully.

### E15 — Error check question Solve 5 12 1

**Type:** Multiple choice

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Error-check question: Solve 5/12 + 1/6 + 3/4 and write the answer as a mixed numeral if needed. Estimate first, then enter the exact answer.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing error check solve write mixed numeral needed estimate.

### E16 — Intermediate digital question A recipe needs 1

**Type:** Short response

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Intermediate digital question: A recipe needs 1 1/2 cups oats and 3/4 cup coconut. How much more oats than coconut? Check the common trap before answering.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing recipe needs cups oats coconut much more oats.

### E17 — Multi step online task Find two different

**Type:** Matching

**Delivery:** Online, provide movable cards with drag, tap-to-place and keyboard controls. Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Multi-step online task: Find two different fraction pairs with related denominators that add to 1 1/4. Use two steps and choose the matching reasoning.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Each card must be matched to the representation with the same value or meaning.

**Coverage:** Online Year 5 task assessing multi step online task find different fraction pairs.

### E18 — Reasoning check Explain when you can add

**Type:** Multiple choice

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Reasoning check: Explain when you can add numerators directly and when you must rename fractions first. Interpret the answer in the story context.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing reasoning check when numerators directly when must rename.

### E19 — Model based question Solve 2 7 10

**Type:** Multiple choice

**Delivery:** Online, provide movable cards with drag, tap-to-place and keyboard controls. Show the required on-screen visual, model or widget; include an accessible text alternative. Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Model-based question: Solve 2 - 7/10 using tenths, then explain the whole-number regrouping. Choose the model or method that matches your answer.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing model based solve tenths whole number regrouping model.

### E20 — Error check question A garden has 3

**Type:** Multiple choice

**Question:** Error-check question: A garden has 3/5 planted with vegetables and 1/4 with flowers. What fraction is planted? What fraction is unplanted? Estimate first, then enter the exact answer.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing error check garden planted vegetables flowers fraction planted.

## Section C — Extended response and analysis

### E21 — Intermediate digital question Calculate 11 12 2

**Type:** Multiple choice

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Intermediate digital question: Calculate 11/12 - 2/3. Then choose the diagram that proves the subtraction correctly.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing calculate diagram proves subtraction correctly.

### E22 — Multi step online task Write a word

**Type:** Matching

**Delivery:** Online, provide movable cards with drag, tap-to-place and keyboard controls. Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Multi-step online task: Write a word problem represented by 1 3/8 + 7/8. Solve it. Use two steps and choose the matching reasoning.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Each card must be matched to the representation with the same value or meaning.

**Coverage:** Online Year 5 task assessing multi step online task write word problem represented.

### E23 — Reasoning check Find the error 4 5

**Type:** Multiple choice

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Reasoning check: Find the error: 4/5 - 1/10 = 3/5. Correct it. Interpret the answer in the story context.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing reasoning check find error correct interpret story context.

### E24 — Model based question Solve 2 5 6

**Type:** Multiple choice

**Delivery:** Online, provide movable cards with drag, tap-to-place and keyboard controls. Show the required on-screen visual, model or widget; include an accessible text alternative. Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Model-based question: Solve 2 5/6 + 1 3/4. Explain how you handled the denominators. Choose the model or method that matches your answer.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing model based solve handled denominators model method matches.

### E25 — Error check question A trail is 4

**Type:** Multiple choice

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Error-check question: A trail is 4 km. You walk 1 2/5 km then 1 3/10 km. How much remains? Estimate first, then enter the exact answer.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing error check trail walk much remains estimate first.

### E26 — Intermediate digital question Make a common denominator

**Type:** Short response

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Intermediate digital question: Make a common denominator table for halves, quarters, eighths and sixteenths. Check the common trap before answering.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing make common denominator table halves quarters eighths sixteenths.

### E27 — Multi step online task Find the missing

**Type:** Matching

**Delivery:** Online, provide movable cards with drag, tap-to-place and keyboard controls. Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Multi-step online task: Find the missing sign: 3/4 + 1/8 ___ 5/6. Justify. Use two steps and choose the matching reasoning.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Each card must be matched to the representation with the same value or meaning.

**Coverage:** Online Year 5 task assessing multi step online task find missing sign justify.

### E28 — Reasoning check Solve 1 1 3 5

**Type:** Multiple choice

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Reasoning check: Solve 1 1/3 - 5/12, then estimate first to check reasonableness. Interpret the answer in the story context.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing reasoning check solve estimate first check reasonableness interpret.

### E29 — Model based question Complete the structured template

**Type:** Multiple choice

**Delivery:** Online, provide movable cards with drag, tap-to-place and keyboard controls. Show the required on-screen visual, model or widget; include an accessible text alternative. Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Model-based question: Complete the structured template for a multi-step fraction addition/subtraction problem with related denominators and solve it. Choose the model or method that matches your answer.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing model based multi step fraction addition subtraction problem.

### E30 — Solve 1 3 4 5 8 and

**Type:** Multiple choice

**Delivery:** Online, provide movable cards with drag, tap-to-place and keyboard controls. Show the required on-screen visual, model or widget; include an accessible text alternative. Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Solve 1 3/4 - 5/8 and choose the model that matches the regrouping.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing solve model matches regrouping.
