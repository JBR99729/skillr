---
curriculum_code: AC9M5N01
subject: Mathematics
year_level: Year 5
description: interpret, compare and order numbers with more than 2 decimal places, including numbers greater than one, using place value understanding; represent these on a number line
batch: 1
practice_questions: 68
exam_questions: 30
worksheet_questions: 10
worksheet_practice_selection: 8
worksheet_exam_selection: 2
launch_practice_ids: P01,P18,P35,P52,P09,P26,P43,P60
quick_read_status: authored
review_status: authored
---

# AC9M5N01 — Year 5 Mathematics digital question bank

## 60-second Quick Read

- **Key idea:** This Year 5 skill is about interpret, compare and order numbers with more than 2 decimal places, including numbers greater than one, using place value understanding; represent these on a number line.
- **Method:** Read the question carefully, identify the values or relationships, choose the most efficient strategy, then check whether the answer makes sense in the context.
- **Worked example:** For a screen task, first match each number, model or condition to its mathematical meaning. Then use the tool or answer box only after you have compared the key place values, units, operations or features.
- **Digital model:** Online questions may use movable cards, selectable diagrams, number-line widgets, grid tools, tables, graphs or constrained answer boxes. Units should sit outside numeric input boxes.
- **Common trap:** Do not answer from the surface pattern only. Check the operation, unit, direction, scale, denominator, coordinate order or likelihood before selecting your final answer.

## Practice questions

## Tier 1 — Knowledge and terms

### P01 — Write 3 406 in a place value

**Type:** Multiple choice

**Question:** Write 3.406 in a place value chart and explain the value of each digit.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing write place value chart value each digit.

### P02 — Order these numbers from smallest to largest

**Type:** Sequencing

**Delivery:** Online, provide movable cards with drag, tap-to-place and keyboard controls. Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Order these numbers from smallest to largest: 2.305, 2.35, 2.053, 2.503, 2.350.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. The order must be complete and use the direction requested in the prompt.

**Coverage:** Online Year 5 task assessing order numbers smallest largest.

### P03 — Which is greater 4 708 or 4

**Type:** Multiple choice

**Question:** Which is greater: 4.708 or 4.78? Explain using place value, not just digit counting.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing greater place value just digit counting.

### P04 — Place 1 245 1 25 and 1

**Type:** Multiple choice

**Delivery:** Show the required on-screen visual, model or widget; include an accessible text alternative.

**Question:** Place 1.245, 1.25 and 1.205 on a number line from 1.2 to 1.3. Explain your positions.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing place number line your positions.

### P05 — A joey weighs 2 305 kg and

**Type:** Multiple choice

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** A joey weighs 2.305 kg and a possum weighs 2.35 kg. Which animal is heavier and by how much?

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing joey weighs possum weighs animal heavier much.

### P06 — Fill in the missing decimal 6 47

**Type:** Short response

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Fill in the missing decimal: 6.47 < ___ < 6.48. Give two possible answers.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing fill missing decimal possible answers.

### P07 — Rename 0 7 as hundredths and thousandths

**Type:** Multiple choice

**Question:** Rename 0.7 as hundredths and thousandths. Use this to explain 0.7 divided by 10.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing rename hundredths thousandths this divided.

### P08 — True or false 5 090 is the

**Type:** Multiple choice

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** True or false: 5.090 is the same as 5.09. Explain.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing true false same.

### P09 — A path is 3 275 m long

**Type:** Short response

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** A path is 3.275 m long. Round it to the nearest tenth, hundredth and whole metre.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing path long round nearest tenth hundredth whole metre.

### P10 — Find a decimal halfway between 8 41

**Type:** Multiple choice

**Question:** Find a decimal halfway between 8.41 and 8.42. Explain why it is halfway.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing find decimal halfway between halfway.

### P11 — Compare 0 609 and 0 69 using

**Type:** Short response

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Compare 0.609 and 0.69 using a place value chart.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing compare place value chart.

### P12 — Write three decimals between 12 4 and

**Type:** Short response

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Write three decimals between 12.4 and 12.5, each with three decimal places.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing write three decimals between each three decimal places.

### P13 — A map shows distances of 1 208

**Type:** Sequencing

**Delivery:** Online, provide movable cards with drag, tap-to-place and keyboard controls. Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** A map shows distances of 1.208 km, 1.28 km and 1.802 km. Order the distances from shortest to longest.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. The order must be complete and use the direction requested in the prompt.

**Coverage:** Online Year 5 task assessing shows distances order distances shortest longest.

### P14 — Complete the pattern 0 034 0 044

**Type:** Short response

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Complete the pattern: 0.034, 0.044, 0.054, ___, ___, ___. Describe the rule.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing pattern describe rule.

### P15 — A student says 7 111 is larger

**Type:** Multiple choice

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** A student says 7.111 is larger than 7.9 because 111 is larger than 9. What is the mistake?

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing says larger than because larger than mistake.

### P16 — Represent 2 375 using tenths hundredths and

**Type:** Short response

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Represent 2.375 using tenths, hundredths and thousandths in words.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing represent tenths hundredths thousandths words.

### P17 — Which decimal is closest to 3 6

**Type:** Multiple choice

**Question:** Which decimal is closest to 3.6: 3.572, 3.621 or 3.68? Explain.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing decimal closest.

## Tier 2 — Core skill practice

### P18 — Write a decimal with a 4 in

**Type:** Short response

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Write a decimal with a 4 in the thousandths place, a 9 in the ones place and a 2 in the tenths place.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing write decimal thousandths place ones place tenths place.

### P19 — A plant grew from 0 846 m

**Type:** Short response

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** A plant grew from 0.846 m to 0.902 m. How much did it grow?

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing plant grew much grow.

### P20 — Find the missing signs 0 505 0

**Type:** Short response

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Find the missing signs: 0.505 ___ 0.55, 2.030 ___ 2.003, 9.8 ___ 9.080.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing find missing signs.

### P21 — Use the on screen tool to build

**Type:** Multiple choice

**Delivery:** Show the required on-screen visual, model or widget; include an accessible text alternative. Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Use the on-screen tool to build or select a number line from 0 to 1 and mark 0.125, 0.5 and 0.875.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing tool build number line mark.

### P22 — Write 4 060 4 006 and 4

**Type:** Short response

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Write 4.060, 4.006 and 4.600 in expanded form.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing write expanded form.

### P23 — Complete the structured template for a real

**Type:** Short response

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Complete the structured template for a real-life measurement problem where 1.708 and 1.78 must be compared.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing real life measurement problem must compared.

### P24 — Explain why thousandths are ten times smaller

**Type:** Multiple choice

**Delivery:** Show the required on-screen visual, model or widget; include an accessible text alternative. Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Explain why thousandths are ten times smaller than hundredths using a grid or money model.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing thousandths times smaller than hundredths grid money model.

### P25 — A lab scale shows 4 096 kg

**Type:** Sequencing

**Delivery:** Online, provide movable cards with drag, tap-to-place and keyboard controls. Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** A lab scale shows 4.096 kg, 4.69 kg and 4.609 kg. Order the masses and explain the two most common comparison mistakes.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. The order must be complete and use the direction requested in the prompt.

**Coverage:** Online Year 5 task assessing scale shows order masses most common comparison mistakes.

### P26 — Find three decimals with thousandths between 7

**Type:** Sequencing

**Delivery:** Online, provide movable cards with drag, tap-to-place and keyboard controls.

**Question:** Find three decimals with thousandths between 7.305 and 7.31. Place them in increasing order.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. The order must be complete and use the direction requested in the prompt.

**Coverage:** Online Year 5 task assessing find three decimals thousandths between place them increasing.

### P27 — A runner records 12 408 s in

**Type:** Multiple choice

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** A runner records 12.408 s in heat 1 and 12.48 s in heat 2. Which run was faster, and by how many seconds?

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing runner records heat heat faster many seconds.

### P28 — Order the labels 0 875 0 905

**Type:** Short response

**Delivery:** Online, provide movable cards with drag, tap-to-place and keyboard controls. Show the required on-screen visual, model or widget; include an accessible text alternative.

**Question:** Order the labels 0.875, 0.905, 0.89 and 0.908 on a number line from left to right.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing order labels number line left right.

### P29 — A student says 3 507 is closer

**Type:** Multiple choice

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** A student says 3.507 is closer to 3.6 than 3.55 because 507 is larger than 55. Correct the reasoning.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing says closer than because larger than correct reasoning.

### P30 — Write a decimal that rounds to 6

**Type:** Short response

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Write a decimal that rounds to 6.4 to the nearest tenth but rounds to 6.35 to the nearest hundredth.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing write decimal rounds nearest tenth rounds nearest hundredth.

### P31 — Complete 2 304 2 tenths hundredths thousandths

**Type:** Short response

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Complete: 2.304 = 2 + ___ tenths + ___ hundredths + ___ thousandths. Then write it in expanded decimal form.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing tenths hundredths thousandths write expanded decimal form.

### P32 — The difference between two decimals is 0

**Type:** Multiple choice

**Question:** The difference between two decimals is 0.006. One decimal is 5.42. Give two possible pairs and explain.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing difference between decimals decimal possible pairs.

### P33 — Order 9 099 9 109 9 19

**Type:** Sequencing

**Delivery:** Online, provide movable cards with drag, tap-to-place and keyboard controls. Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Order 9.099, 9.109, 9.19, 9.091 and 9.9 from smallest to largest.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. The order must be complete and use the direction requested in the prompt.

**Coverage:** Online Year 5 task assessing order smallest largest.

### P34 — A path is 3 2 km long

**Type:** Short response

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** A path is 3.2 km long. Mina walked 1.875 km. How much farther must she walk? Show regrouping clearly.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing path long mina walked much farther must walk.

## Tier 3 — Guided application

### P35 — Choose the decimal exactly halfway between 4

**Type:** Multiple choice

**Question:** Choose the decimal exactly halfway between 4.08 and 4.09, then explain why 4.085 has three decimal places.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing decimal exactly halfway between three decimal places.

### P36 — Complete the structured template for a measurement

**Type:** Short response

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Complete the structured template for a measurement story where 0.304 and 0.34 are easily confused, then solve it.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing measurement story easily confused solve.

### P37 — A board is cut into pieces of

**Type:** Short response

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** A board is cut into pieces of 1.205 m, 1.025 m and 1.25 m. What is the total length?

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing board into pieces total length.

### P38 — Find the missing digits 8 7 8

**Type:** Short response

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Find the missing digits: 8.__7 < 8.307 and 8.2__ > 8.28. Give all possible digits if there are any.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing find missing digits possible digits there.

### P39 — Explain why adding a zero to the

**Type:** Multiple choice

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Explain why adding a zero to the right of a decimal can keep the value the same, but adding a zero between digits may change it.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing adding zero right decimal keep value same adding.

### P40 — Round 15 649 to the nearest whole

**Type:** Multiple choice

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Round 15.649 to the nearest whole, tenth and hundredth, then identify which rounded answer is most precise.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing round nearest whole tenth hundredth identify rounded most.

### P41 — A swimmer improved from 38 502 s

**Type:** Short response

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** A swimmer improved from 38.502 s to 38.059 s. How much time did they improve by?

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing swimmer improved much time they improve.

### P42 — Write five decimals between 0 99 and

**Type:** Short response

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Write five decimals between 0.99 and 1.01, including at least two thousandths.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing write five decimals between including least thousandths.

### P43 — Use a place value chart to compare

**Type:** Short response

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Use a place-value chart to compare 6.060, 6.006, 6.600 and 6.066.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing place value chart compare.

### P44 — A digital thermometer shows 21 075 C

**Type:** Short response

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** A digital thermometer shows 21.075°C. What happens to the value if each digit shifts one place to the right?

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing thermometer shows happens value each digit shifts place.

### P45 — Find a decimal greater than 2 705

**Type:** Short response

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Find a decimal greater than 2.705 but less than 2.71 that has an even digit in the thousandths place.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing find decimal greater than less than even digit.

### P46 — Explain whether 0 999 is closer to

**Type:** Multiple choice

**Delivery:** Show the required on-screen visual, model or widget; include an accessible text alternative.

**Question:** Explain whether 0.999 is closer to 1 or to 0.9, using distance on a number line.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing whether closer distance number line.

### P47 — Complete the on screen table showing tenths

**Type:** Short response

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Complete the on-screen table showing tenths, hundredths and thousandths for 0.4, 0.04 and 0.004.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing table showing tenths hundredths thousandths.

### P48 — Select the challenge question where the correct

**Type:** Short response

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Select the challenge question where the correct answer depends on comparing decimals to thousandths, then solve it.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing challenge correct depends comparing decimals thousandths solve.

### P49 — Beginner digital question Write 3 406 in

**Type:** Multiple choice

**Question:** Beginner digital question: Write 3.406 in a place value chart and explain the value of each digit.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing write place value chart value each digit.

### P50 — Use the on screen options Order these

**Type:** Sequencing

**Delivery:** Online, provide movable cards with drag, tap-to-place and keyboard controls. Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Use the on-screen options: Order these numbers from smallest to largest: 2.305, 2.35, 2.053, 2.503, 2.350.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. The order must be complete and use the direction requested in the prompt.

**Coverage:** Online Year 5 task assessing order numbers smallest largest.

### P51 — Complete the answer box Which is greater

**Type:** Multiple choice

**Question:** Complete the answer box: Which is greater: 4.708 or 4.78? Explain using place value, not just digit counting.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing greater place value just digit counting.

## Tier 4 — Problem solving and reasoning

### P52 — Match or choose Place 1 245 1

**Type:** Matching

**Delivery:** Online, provide movable cards with drag, tap-to-place and keyboard controls. Show the required on-screen visual, model or widget; include an accessible text alternative.

**Question:** Match or choose: Place 1.245, 1.25 and 1.205 on a number line from 1.2 to 1.3. Explain your positions.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Each card must be matched to the representation with the same value or meaning.

**Coverage:** Online Year 5 task assessing match place number line your positions.

### P53 — Select the best answer A joey weighs

**Type:** Multiple choice

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Select the best answer: A joey weighs 2.305 kg and a possum weighs 2.35 kg. Which animal is heavier and by how much?

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing best joey weighs possum weighs animal heavier much.

### P54 — Beginner digital question Fill in the missing

**Type:** Short response

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Beginner digital question: Fill in the missing decimal: 6.47 < ___ < 6.48. Give two possible answers.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing fill missing decimal possible answers.

### P55 — Use the on screen options Rename 0

**Type:** Multiple choice

**Question:** Use the on-screen options: Rename 0.7 as hundredths and thousandths. Use this to explain 0.7 divided by 10.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing rename hundredths thousandths this divided.

### P56 — Complete the answer box True or false

**Type:** Multiple choice

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Complete the answer box: True or false: 5.090 is the same as 5.09. Explain.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing true false same.

### P57 — Match or choose A path is 3

**Type:** Matching

**Delivery:** Online, provide movable cards with drag, tap-to-place and keyboard controls. Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Match or choose: A path is 3.275 m long. Round it to the nearest tenth, hundredth and whole metre.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Each card must be matched to the representation with the same value or meaning.

**Coverage:** Online Year 5 task assessing match path long round nearest tenth hundredth whole.

### P58 — Select the best answer Find a decimal

**Type:** Multiple choice

**Question:** Select the best answer: Find a decimal halfway between 8.41 and 8.42. Explain why it is halfway.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing best find decimal halfway between halfway.

### P59 — Beginner digital question Compare 0 609 and

**Type:** Short response

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Beginner digital question: Compare 0.609 and 0.69 using a place value chart.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing compare place value chart.

### P60 — Use the on screen options Write three

**Type:** Short response

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Use the on-screen options: Write three decimals between 12.4 and 12.5, each with three decimal places.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing write three decimals between each three decimal places.

### P61 — Complete the answer box A map shows

**Type:** Sequencing

**Delivery:** Online, provide movable cards with drag, tap-to-place and keyboard controls. Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Complete the answer box: A map shows distances of 1.208 km, 1.28 km and 1.802 km. Order the distances from shortest to longest.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. The order must be complete and use the direction requested in the prompt.

**Coverage:** Online Year 5 task assessing shows distances order distances shortest longest.

### P62 — Match or choose Complete the pattern 0

**Type:** Matching

**Delivery:** Online, provide movable cards with drag, tap-to-place and keyboard controls.

**Question:** Match or choose: Complete the pattern: 0.034, 0.044, 0.054, ___, ___, ___. Describe the rule.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Each card must be matched to the representation with the same value or meaning.

**Coverage:** Online Year 5 task assessing match pattern describe rule.

### P63 — Select the best answer A student says

**Type:** Multiple choice

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Select the best answer: A student says 7.111 is larger than 7.9 because 111 is larger than 9. What is the mistake?

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing best says larger than because larger than mistake.

### P64 — Beginner digital question Represent 2 375 using

**Type:** Short response

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Beginner digital question: Represent 2.375 using tenths, hundredths and thousandths in words.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing represent tenths hundredths thousandths words.

### P65 — Use the on screen options Which decimal

**Type:** Multiple choice

**Question:** Use the on-screen options: Which decimal is closest to 3.6: 3.572, 3.621 or 3.68? Explain.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing decimal closest.

### P66 — Complete the answer box Write a decimal

**Type:** Short response

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Complete the answer box: Write a decimal with a 4 in the thousandths place, a 9 in the ones place and a 2 in the tenths place.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing write decimal thousandths place ones place tenths place.

### P67 — Match or choose A plant grew from

**Type:** Matching

**Delivery:** Online, provide movable cards with drag, tap-to-place and keyboard controls. Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Match or choose: A plant grew from 0.846 m to 0.902 m. How much did it grow?

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Each card must be matched to the representation with the same value or meaning.

**Coverage:** Online Year 5 task assessing match plant grew much grow.

### P68 — Select the best answer Find the missing

**Type:** Multiple choice

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Select the best answer: Find the missing signs: 0.505 ___ 0.55, 2.030 ___ 2.003, 9.8 ___ 9.080.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing best find missing signs.

## Exam questions

## Section A — Core knowledge

### E01 — Beginner digital question use the on screen

**Type:** Multiple choice

**Delivery:** Show the required on-screen visual, model or widget; include an accessible text alternative. Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Beginner digital question: use the on-screen tool to build or select a number line from 0 to 1 and mark 0.125, 0.5 and 0.875.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing tool build number line mark.

### E02 — Use the on screen options Write 4

**Type:** Short response

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Use the on-screen options: Write 4.060, 4.006 and 4.600 in expanded form.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing write expanded form.

### E03 — Complete the answer box Complete the structured

**Type:** Short response

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Complete the answer box: Complete the structured template for a real-life measurement problem where 1.708 and 1.78 must be compared.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing real life measurement problem must compared.

### E04 — Match or choose Explain why thousandths are

**Type:** Matching

**Delivery:** Online, provide movable cards with drag, tap-to-place and keyboard controls. Show the required on-screen visual, model or widget; include an accessible text alternative. Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Match or choose: Explain why thousandths are ten times smaller than hundredths using a grid or money model.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Each card must be matched to the representation with the same value or meaning.

**Coverage:** Online Year 5 task assessing match thousandths times smaller than hundredths grid money.

### E05 — Order 3 4 3 04 and 3

**Type:** Short response

**Delivery:** Online, provide movable cards with drag, tap-to-place and keyboard controls. Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Order 3.4, 3.04 and 3.404 from smallest to largest.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing order smallest largest.

### E06 — Intermediate digital question A lab scale shows

**Type:** Sequencing

**Delivery:** Online, provide movable cards with drag, tap-to-place and keyboard controls. Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Intermediate digital question: A lab scale shows 4.096 kg, 4.69 kg and 4.609 kg. Order the masses and explain the two most common comparison mistakes. Check the common trap before answering.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. The order must be complete and use the direction requested in the prompt.

**Coverage:** Online Year 5 task assessing scale shows order masses most common comparison mistakes.

### E07 — Multi step online task Find three decimals

**Type:** Sequencing

**Delivery:** Online, provide movable cards with drag, tap-to-place and keyboard controls. Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Multi-step online task: Find three decimals with thousandths between 7.305 and 7.31. Place them in increasing order. Use two steps and choose the matching reasoning.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. The order must be complete and use the direction requested in the prompt.

**Coverage:** Online Year 5 task assessing multi step online task find three decimals thousandths.

### E08 — Reasoning check A runner records 12 408

**Type:** Multiple choice

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Reasoning check: A runner records 12.408 s in heat 1 and 12.48 s in heat 2. Which run was faster, and by how many seconds? Interpret the answer in the story context.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing reasoning check runner records heat heat faster many.

### E09 — Model based question Order the labels 0

**Type:** Short response

**Delivery:** Online, provide movable cards with drag, tap-to-place and keyboard controls. Show the required on-screen visual, model or widget; include an accessible text alternative.

**Question:** Model-based question: Order the labels 0.875, 0.905, 0.89 and 0.908 on a number line from left to right.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing model based order labels number line left right.

### E10 — Error check question A student says 3

**Type:** Multiple choice

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Error-check question: A student says 3.507 is closer to 3.6 than 3.55 because 507 is larger than 55. Correct the reasoning. Estimate first, then enter the exact answer.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing error check says closer than because larger than.

## Section B — Application and problem solving

### E11 — Intermediate digital question Write a decimal that

**Type:** Short response

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Intermediate digital question: Write a decimal that rounds to 6.4 to the nearest tenth but rounds to 6.35 to the nearest hundredth. Check the common trap before answering.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing write decimal rounds nearest tenth rounds nearest hundredth.

### E12 — Multi step online task Complete 2 304

**Type:** Matching

**Delivery:** Online, provide movable cards with drag, tap-to-place and keyboard controls. Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Multi-step online task: Complete: 2.304 = 2 + ___ tenths + ___ hundredths + ___ thousandths. Then write it in expanded decimal form. Use two steps and choose the matching reasoning.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Each card must be matched to the representation with the same value or meaning.

**Coverage:** Online Year 5 task assessing multi step online task tenths hundredths thousandths write.

### E13 — Reasoning check The difference between two decimals

**Type:** Multiple choice

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Reasoning check: The difference between two decimals is 0.006. One decimal is 5.42. Give two possible pairs and explain. Interpret the answer in the story context.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing reasoning check difference between decimals decimal possible pairs.

### E14 — Model based question Order 9 099 9

**Type:** Sequencing

**Delivery:** Online, provide movable cards with drag, tap-to-place and keyboard controls. Show the required on-screen visual, model or widget; include an accessible text alternative. Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Model-based question: Order 9.099, 9.109, 9.19, 9.091 and 9.9 from smallest to largest. Choose the model or method that matches your answer.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. The order must be complete and use the direction requested in the prompt.

**Coverage:** Online Year 5 task assessing model based order smallest largest model method matches.

### E15 — Error check question A path is 3

**Type:** Multiple choice

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Error-check question: A path is 3.2 km long. Mina walked 1.875 km. How much farther must she walk? Show regrouping clearly. Estimate first, then enter the exact answer.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing error check path long mina walked much farther.

### E16 — Intermediate digital question Choose the decimal exactly

**Type:** Multiple choice

**Question:** Intermediate digital question: Choose the decimal exactly halfway between 4.08 and 4.09, then explain why 4.085 has three decimal places. Check the common trap before answering.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing decimal exactly halfway between three decimal places check.

### E17 — Multi step online task Complete the structured

**Type:** Matching

**Delivery:** Online, provide movable cards with drag, tap-to-place and keyboard controls. Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Multi-step online task: Complete the structured template for a measurement story where 0.304 and 0.34 are easily confused, then solve it. Use two steps and choose the matching reasoning.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Each card must be matched to the representation with the same value or meaning.

**Coverage:** Online Year 5 task assessing multi step online task measurement story easily confused.

### E18 — Reasoning check A board is cut into

**Type:** Multiple choice

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Reasoning check: A board is cut into pieces of 1.205 m, 1.025 m and 1.25 m. What is the total length? Interpret the answer in the story context.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing reasoning check board into pieces total length interpret.

### E19 — Model based question Find the missing digits

**Type:** Multiple choice

**Delivery:** Online, provide movable cards with drag, tap-to-place and keyboard controls. Show the required on-screen visual, model or widget; include an accessible text alternative. Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Model-based question: Find the missing digits: 8.__7 < 8.307 and 8.2__ > 8.28. Give all possible digits if there are any. Choose the model or method that matches your answer.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing model based find missing digits possible digits there.

### E20 — Error check question Explain why adding a

**Type:** Multiple choice

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Error-check question: Explain why adding a zero to the right of a decimal can keep the value the same, but adding a zero between digits may change it. Estimate first, then enter the exact answer.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing error check adding zero right decimal keep value.

## Section C — Extended response and analysis

### E21 — Intermediate digital question Round 15 649 to

**Type:** Multiple choice

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Intermediate digital question: Round 15.649 to the nearest whole, tenth and hundredth, then identify which rounded answer is most precise. Check the common trap before answering.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing round nearest whole tenth hundredth identify rounded most.

### E22 — Multi step online task A swimmer improved

**Type:** Matching

**Delivery:** Online, provide movable cards with drag, tap-to-place and keyboard controls. Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Multi-step online task: A swimmer improved from 38.502 s to 38.059 s. How much time did they improve by? Use two steps and choose the matching reasoning.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Each card must be matched to the representation with the same value or meaning.

**Coverage:** Online Year 5 task assessing multi step online task swimmer improved much time.

### E23 — Reasoning check Write five decimals between 0

**Type:** Multiple choice

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Reasoning check: Write five decimals between 0.99 and 1.01, including at least two thousandths. Interpret the answer in the story context.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing reasoning check write five decimals between including least.

### E24 — Model based question Use a place value

**Type:** Multiple choice

**Delivery:** Online, provide movable cards with drag, tap-to-place and keyboard controls. Show the required on-screen visual, model or widget; include an accessible text alternative. Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Model-based question: Use a place-value chart to compare 6.060, 6.006, 6.600 and 6.066. Choose the model or method that matches your answer.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing model based place value chart compare model method.

### E25 — Error check question A digital thermometer shows

**Type:** Multiple choice

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Error-check question: A digital thermometer shows 21.075°C. What happens to the value if each digit shifts one place to the right? Estimate first, then enter the exact answer.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing error check thermometer shows happens value each digit.

### E26 — Intermediate digital question Find a decimal greater

**Type:** Short response

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Intermediate digital question: Find a decimal greater than 2.705 but less than 2.71 that has an even digit in the thousandths place. Check the common trap before answering.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing find decimal greater than less than even digit.

### E27 — Multi step online task Explain whether 0

**Type:** Matching

**Delivery:** Online, provide movable cards with drag, tap-to-place and keyboard controls. Show the required on-screen visual, model or widget; include an accessible text alternative. Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Multi-step online task: Explain whether 0.999 is closer to 1 or to 0.9, using distance on a number line. Use two steps and choose the matching reasoning.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Each card must be matched to the representation with the same value or meaning.

**Coverage:** Online Year 5 task assessing multi step online task whether closer distance number.

### E28 — Reasoning check Complete the on screen table

**Type:** Multiple choice

**Delivery:** Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Reasoning check: Complete the on-screen table showing tenths, hundredths and thousandths for 0.4, 0.04 and 0.004. Interpret the answer in the story context.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing reasoning check table showing tenths hundredths thousandths interpret.

### E29 — Model based question Select the challenge question

**Type:** Multiple choice

**Delivery:** Online, provide movable cards with drag, tap-to-place and keyboard controls. Show the required on-screen visual, model or widget; include an accessible text alternative. Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** Model-based question: Select the challenge question where the correct answer depends on comparing decimals to thousandths, then solve it. Choose the model or method that matches your answer.

A. Correct response
B. Place-value or calculation distractor
C. Incomplete response
D. Reversed reasoning

**Correct answer:** A. Correct response; configure the final platform option text to match the authored answer.

**Coverage:** Online Year 5 task assessing model based challenge correct depends comparing decimals thousandths.

### E30 — A cyclist records 8 075 km 8

**Type:** Short response

**Delivery:** Online, provide movable cards with drag, tap-to-place and keyboard controls. Place units outside numeric answer boxes and accept equivalent spacing or decimal formats.

**Question:** A cyclist records 8.075 km, 8.507 km and 8.57 km. Order them and identify the closest pair.

**Marking key:** Award credit for the mathematically correct response and for satisfying every stated condition. Use the platform answer key to store the exact accepted value, equivalent value or selected option.

**Coverage:** Online Year 5 task assessing cyclist records order them identify closest pair.
