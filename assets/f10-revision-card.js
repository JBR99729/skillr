(() => {
  ...
  function safeVisual(doc) {
    ...
    const candidates = ['#examples figure','#teaching figure','.math-concept-diagram','.science-concept-diagram','.curriculum-detail-body figure','.lesson-part figure','.concept-visual','.worked-example svg'];
    let source = null;
    ...
    clone.querySelectorAll('img').forEach(img=>{ const raw=img.getAttribute('src')||''; if(raw) img.src=new URL(raw,topicLink.href).href; img.removeAttribute('srcset'); img.removeAttribute('sizes'); img.loading='eager'; img.decoding='async'; });
    return clone.outerHTML;
  }

  function fromTopic(doc) {
    const sectionText = (selector) => clean(doc.querySelector(selector)?.textContent);
    const firstText = (selectors) => { for (const selector of selectors) { const el=doc.querySelector(selector); if(el&&clean(el.textContent)) return clean(el.textContent); } return ''; };
  ...
  function render(data) {
    ...
    const box=document.createElement('section'); box.className='pre-read-notes skillr-revision-card'; box.dataset.skillrRevisionCard='true';
