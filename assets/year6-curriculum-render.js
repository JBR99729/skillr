(() => {
  ...
  const match = location.pathname.match(/^\/year6\/(maths|science|english)\/(ac9[mse]6[a-z0-9]+)/i);
  if (!match) return;
  ...
  const units = window[`SkillrYear6${subjectName}Data`] || {};
  const order = window[`SkillrYear6${subjectName}Order`] || Object.keys(units);
  ...
  function relatedLinks() {
    return order.map((unitCode) => {
      ...
      return related ? `<li><a href="/year6/${subject}/${related.slug}/">${unitCode}: ${esc(related.title)}</a></li>` : "";
    }).join("");
  }
  ...
  function render() {
    ...
    document.title = `${code} ${unit.title} | Year 6 ${subjectName}`;
    const description = q('meta[name="description"]');
    if (description) description.content = `Teach ${code} ${unit.desc} with a connected visual Year 6 ${subjectName} lesson, teacher slides, worksheets, Practice and Test.`;

    hero.classList.add("micro-hero");
    hero.innerHTML = `<p class="curriculum-eyebrow">${code} • Year 6 ${subjectName}</p><h1>${esc(unit.title)}</h1><p class="micro-hero__subtitle">${esc(unit.subtitle)}</p><p class="micro-hero__goal">${esc(unit.learn)}</p><div class="hero-visual">${unit.hero_visual}</div><p class="no-stock-note">Instructional diagrams are lightweight page graphics; this lesson does not rely on stock photographs.</p><div class="topic-action-row"><a class="primary" href="#teaching-lesson">Start lesson</a><a href="/year6/${subject}/${unit.slug}/teacher-slides/" target="_blank" rel="noopener">Teacher slides</a><a href="/quiz/year-6/${routeSubject}/${code.toLowerCase()}/worksheet/" target="_blank" rel="noopener">Worksheets</a><a href="/quiz/year-6/${routeSubject}/${code.toLowerCase()}/practice/">Practice</a><a href="/quiz/year-6/${routeSubject}/${code.toLowerCase()}/test/">Test</a></div><button class="report-issue-button" type="button" data-report-issue>Report issue</button>`;
    ...
    const reasoning = subject === "maths"
      ? [["Represent","What quantities, structures and constraints are visible?"],["Reason","Why does the property or strategy work?"],["Verify","Which estimate, inverse or alternate representation confirms it?"]]
      : subject === "science"
        ? [["Observe","What can be measured, classified or compared?"],["Explain","How does the model account for the evidence?"],["Evaluate","What variables, errors, limits or questions remain?"]]
        : [["Notice","Which language, structural or multimodal choices matter?"],["Explain","How do they shape meaning and audience position?"],["Transfer","How can the feature be applied and reviewed in a new text?"]];

    const lesson = `<details class="topic-menu combined-lesson-menu" id="teaching-lesson" open><summary><span class="menu-title">${lessonTitle}</span></summary><div class="menu-content combined-lesson-content"><section class="lesson-part"><h3>Curriculum focus and lesson scope</h3><div class="curriculum-focus"><strong>${code}:</strong><p>${esc(unit.desc)}</p><div class="lesson-timing"><span>Visual model: 10–15 min</span><span>Guided reasoning: 10 min</span><span>Activity/investigation: 15 min</span><span>Independent check: 5–10 min</span></div></div></section><section class="lesson-part"><h3>Learning intention and success criteria</h3><div class="success-grid"><div class="success-card"><h4>Learning intention</h4><p>${esc(unit.learn)}</p></div><div class="success-card"><h4>I can…</h4><ul>${successItems}</ul></div></div></section><section class="lesson-part"><h3>Learning routine</h3><div class="lesson-callout">${esc(unit.routine)}</div></section><section class="lesson-part"><h3>Teach It</h3><p><strong>${esc(unit.model_title)}</strong></p>${unit.model_html}<div class="reasoning-strip">${reasoning.map(([title,text])=>`<div><strong>${title}</strong><span>${text}</span></div>`).join("")}</div></section><section class="lesson-part"><h3>Apply and Transfer</h3><p><strong>${esc(unit.apply_title)}</strong></p>${unit.apply_html}</section><section class="lesson-part"><h3>Guided and collaborative activities</h3><div class="mini-grid-3">${activities}</div></section><section class="lesson-part"><h3>Common Mix-Ups</h3><div class="mini-grid-4">${mixups}</div></section><section class="lesson-part"><h3>Quick Check</h3><div class="mini-grid-4">${checks}</div></section></div></details>`;

    main.className = "curriculum-layout lesson-layout";
    main.innerHTML = `<div class="lesson-stack">${lesson}<details class="topic-menu"><summary><span class="menu-title">Quick Mastery Check</span></summary><div class="menu-content"><p><strong>Move to Practice or Test when the student can use the concept independently and justify the response with an appropriate model, evidence or textual feature.</strong></p><div class="mastery-row">${mastery}</div></div></details>${menu("Australian Curriculum description & elaborations","Reference",curriculum)}${menu("Teacher resource","Resource",`<h3>${code} live classroom teacher slides</h3><p>Use the core visual lesson and any additional connected-concept slides generated from the curriculum elaborations.</p><a class="curriculum-button primary" href="/year6/${subject}/${unit.slug}/teacher-slides/" target="_blank" rel="noopener">Open teacher slides</a>`,"teacher-slide")}${menu("International curriculum mapping","Links",international)}${menu(`Related Year 6 ${subjectName} topics`,"Links",`<ul class="curriculum-related-list">${relatedLinks()}</ul>`)}${menu("Official references","Sources",official)}<section class="lesson-part"><div class="curriculum-focus"><strong>Need more detail on this topic?</strong><p>Email <a href="mailto:skillrhublearning@gmail.com">skillrhublearning@gmail.com</a> with the part you would like explained further. SkillrHub will use that feedback to expand this section.</p></div></section></div><aside class="curriculum-sidebar"><section class="curriculum-panel"><h2>Next step</h2><div class="curriculum-link-row"><a class="curriculum-button primary" href="/quiz/year-6/${routeSubject}/${code.toLowerCase()}/practice/">Practice</a><a class="curriculum-button" href="/quiz/year-6/${routeSubject}/${code.toLowerCase()}/worksheet/" target="_blank" rel="noopener">Worksheets</a><a href="/quiz/year-6/${routeSubject}/${code.toLowerCase()}/test/">Test</a></div></section><section class="curriculum-panel"><h2>Learning path</h2><div class="curriculum-link-row"><a href="/year5/">Previous year</a><a href="/year7/">Next year</a><a href="/year6/curriculum/${subject}/">All Year 6 ${subjectName}</a></div></section></aside>`;
