(() => {
  ...
  function apply() {
    ...
    const elaborations = (unit.elaborations || []).map((item) => `<span>${esc(item.label)}: ${esc(item.title)}</span>`).join("");
    notes.innerHTML = `<h2>60-second Quick Read</h2><div class="foundation-test-visuals"><div class="foundation-test-visual"><h3>See the idea</h3>${concept || model}</div><div class="foundation-test-visual"><h3>Use the idea</h3>${apply || model}</div></div>${elaborations ? `<div class="foundation-test-elaborations">${elaborations}</div>` : ""}<div class="foundation-test-points"><div><strong>Say</strong>${esc(unit.learn || unit.desc || "Explain the main idea in your own words.")}</div><div><strong>Show</strong>${esc(unit.model_title || "Use the visual model and point to the important parts.")}</div><div><strong>Check</strong>${esc(mistake ? `${mistake[0]} — ${mistake[1]}` : unit.quick?.[0] || "Check the answer against the model.")}</div></div>`;
