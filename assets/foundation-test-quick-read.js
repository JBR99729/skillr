(() => {
  ...
  function apply() {
    ...
    if (intro) intro.textContent = "Read the key idea, recall the curriculum points, then begin the Test.";
    ...
    const keyVisual = concept || unit.hero_visual || "";
    const mistake = unit.mistakes?.[0];
    const elaborationList = unit.elaborations || [];
    const elaborations = elaborationList.map((item) => `<span>${esc(item.label)}: ${esc(item.title)}</span>`).join("");
    const remember = elaborationList[0]
      ? `${elaborationList[0].label}: ${elaborationList[0].title}`
      : unit.quick?.[0] || "Name the important mathematical idea before answering.";
    notes.innerHTML = `<h2>60-second Quick Read</h2>${keyVisual ? `<div class="foundation-test-visuals"><div class="foundation-test-visual"><h3>Key visual</h3>${keyVisual}</div></div>` : ""}${elaborations ? `<div class="foundation-test-elaborations">${elaborations}</div>` : ""}<div class="foundation-test-points"><div><strong>Core idea</strong>${esc(unit.learn || unit.desc || "Explain the main idea in your own words.")}</div><div><strong>Remember</strong>${esc(remember)}</div><div><strong>Check</strong>${esc(mistake ? `${mistake[0]} — ${mistake[1]}` : unit.quick?.[0] || "Check that the answer matches the key idea.")}</div></div>`;
