(() => {
  ...
  // Marked AC9E4LA01–LA12, LE01–LE05 and LY01 pages own their static homework and dedicated PDF banks.
  if (match[1].toLowerCase() === "english" && /^ac9e4(?:la(?:0[1-9]|1[0-2])|le0[1-5]|ly01)$/i.test(match[2]) && document.body?.getAttribute("data-skillr-authored-worksheet") === "true") return;
  const subject = match[1].toLowerCase();
  const subjectName = subject === "science" ? "Science" : "English";
  const code = match[2].toUpperCase();
  const unit = window[`SkillrYear4${subjectName}Data`]?.[code];
  const worksheet = window[`SkillrYear4${subjectName}WorksheetData`]?.[code];
  ...
  function ensureCss() {
    ...
    if (document.getElementById("skillr-year4-subject-worksheet-css")) return;
    const style = document.createElement("style");
    style.id = "skillr-year4-subject-worksheet-css";
    style.textContent = `
  ...
  function renderScreen() {
    ...
    const evidenceText = subject === "science" ? "Use observations, measurements, models or fair-test reasoning when the question asks you to explain." : "Use specific language, structure, visual or text evidence when the question asks you to explain.";
    document.body.innerHTML = `<div class="worksheet-shell"><nav class="worksheet-nav" aria-label="Breadcrumb"><a href="/">Home</a><a href="/year4/curriculum/${subject}/">Year 4 ${subjectName}</a><a href="/year4/${subject}/${unit.slug}/">${code} topic</a></nav><header class="worksheet-hero"><div class="brandbar"><div class="brandmark">SkillrHub <span>F–10</span></div><small>Year 4 ${subjectName} • Student Worksheet</small></div><p class="eyebrow">${code} • Year 4 ${subjectName}</p><h1>${esc(worksheet.title)} Worksheet</h1><p>The questions consolidate the visual models, connected curriculum ideas and misconceptions taught in the topic guide.</p><div class="worksheet-meta"><span>8 core questions</span><span>Separate 2-question enrichment</span><span>Additional strand sheets where needed</span><span>US Letter portrait</span><span>Preview before download</span></div><div class="worksheet-actions"><button class="primary" id="previewCorePdfButton" type="button">Preview Worksheet 1 — Core</button><button class="secondary" id="previewExtensionPdfButton" type="button">Preview enrichment extension</button><a href="/year4/${subject}/${unit.slug}/">Back to topic</a><a href="/quiz/year-4/${subject}/${code.toLowerCase()}/practice/">Open Practice</a></div><div class="worksheet-print-tip">Worksheets may use a second page to retain readable text and sufficient response space. Use double-sided printing to save paper.</div></header><main id="worksheetRoot"><section class="worksheet-paper worksheet-core">${watermark()}${paperHeader(`${code} — ${esc(worksheet.title)}`)}<p class="reasoning-reminder">${esc(evidenceText)}</p><section class="core-grid">${core.map((question, i) => renderQuestion(question, i)).join("")}</section><footer class="worksheet-footer"><span><strong>SkillrHub F–10</strong> • Year 4 ${subjectName} Core Worksheet</span><span>skillrhub.com</span></footer></section><section class="worksheet-paper worksheet-extension">${watermark()}${paperHeader(`${code} — Enrichment Extension`, "Optional reasoning, analysis and creation challenges")}<p class="extension-note">Use this extension for students ready to compare, evaluate, create or investigate beyond the core examples.</p><section class="enrichment-grid">${enrichment.map((question, i) => renderQuestion(question, i + core.length)).join("")}</section><footer class="worksheet-footer"><span><strong>SkillrHub F–10</strong> • Optional Enrichment Extension</span><span>skillrhub.com</span></footer></section></main></div>`;
  }
  ...
  async function previewPdf(mode) {
    ...
    try {
      ...
      const header = () => {
        ...
        doc.setFont("helvetica","normal"); doc.setFontSize(7.8); doc.text(subject === "science" ? "Use relevant evidence and precise scientific vocabulary." : "Use relevant text, language or visual evidence.",margin,34);
      ...
      const footer = () => {
        ...
        doc.text(`SkillrHub F-10 • Year 4 ${subjectName} ${extension ? "Enrichment" : "Core"}`,margin,pageH-8);
  ...
  if (meta) meta.content = `${code} Year 4 ${subjectName} worksheet with 8 core questions, enrichment and connected-strand worksheets where required.`;
  renderScreen();
