(() => {
  ...
  function render() {
    ...
    hero.innerHTML = `<p class="curriculum-eyebrow">${code} • Year 5 ${subjectName}</p><h1>${esc(unit.title)}</h1><p class="micro-hero__subtitle">${esc(unit.subtitle)}</p><p class="micro-hero__goal">${esc(unit.learn)}</p><div class="hero-visual">${unit.hero_visual}</div><div class="topic-action-row"><a class="primary" href="#teaching-lesson">Start lesson</a><a href="/worksheets/year5/${subject}/teacher-slides/live.html?code=${code}" target="_blank" rel="noopener">Teacher slides</a><a href="/quiz/year-5/${routeSubject}/${code.toLowerCase()}/worksheet/" target="_blank" rel="noopener">Worksheets</a><a href="/quiz/year-5/${routeSubject}/${code.toLowerCase()}/practice/">Practice</a><a href="/quiz/year-5/${routeSubject}/${code.toLowerCase()}/test/">Test</a></div><button class="report-issue-button" type="button" data-report-issue>Report issue</button>`;
