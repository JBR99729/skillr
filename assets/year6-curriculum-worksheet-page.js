(() => {
  ...
  const match = location.pathname.match(/^\/quiz\/year-6\/(math|science|english)\/(ac9[mse]6[a-z0-9]+)\/worksheet\/?$/i);
  if (!match) return;
  ...
  const worksheet = window[`SkillrYear6${subjectName}WorksheetData`]?.[code];
  ...
  function ensureStyles() {
    ...
    if (document.getElementById("skillr-year6-worksheet-style")) return;
    const style = document.createElement("style");
    style.id = "skillr-year6-worksheet-style";
    style.textContent = `
      body{background:#f4f7fb}.worksheet-shell{max-width:1120px;margin:0 auto;padding:18px}.worksheet-nav{display:flex;flex-wrap:wrap;gap:8px;margin-bottom:12px}.worksheet-nav a{color:#2457d6;font-weight:800;text-decoration:none}.worksheet-hero{border:1px solid #d8e2ef;border-radius:18px;background:#fff;padding:20px;box-shadow:0 8px 24px rgba(28,55,91,.08)}.worksheet-hero h1{font-size:clamp(1.45rem,3vw,2.05rem);margin:.25rem 0}.worksheet-meta{display:flex;flex-wrap:wrap;gap:8px;margin:12px 0}.worksheet-meta span{border:1px solid #d7e3fb;background:#f7faff;border-radius:999px;padding:6px 9px;font-size:.82rem;font-weight:800}.worksheet-actions{display:flex;flex-wrap:wrap;gap:8px;margin-top:12px}.worksheet-actions a,.worksheet-actions button{border:0;border-radius:12px;padding:10px 14px;font-weight:900;text-decoration:none;cursor:pointer}.worksheet-actions .primary{background:#2457d6;color:#fff}.worksheet-actions .secondary{background:#173968;color:#fff}.worksheet-actions a{background:#edf2f7;color:#173968}.worksheet-print-tip{margin-top:10px;color:#5d6c80;font-size:.9rem}.brandbar{display:flex;justify-content:space-between;gap:12px;align-items:center}.brandmark{font-weight:900;color:#2457d6}.brandmark span{color:#173968}.worksheet-paper,.worksheet-question,.question-prompt,.worksheet-options span,.match-grid p{color:#203047}.worksheet-paper{position:relative;overflow:hidden;background:#fff;border:1.5px solid #2457d6!important}.worksheet-paper+.worksheet-paper{margin-top:18px}.worksheet-extension{border-style:dashed!important}.worksheet-question{break-inside:avoid}.worksheet-question .question-line{display:flex;align-items:flex-start;gap:6px}.worksheet-question .question-prompt{margin:0;flex:1}.question-number-text{font-weight:900;color:#173968}.worksheet-options{display:flex!important;flex-wrap:wrap;gap:8px 14px;margin:6px 0}.worksheet-options span{white-space:normal}.fill-template{font-weight:800;color:#173968;margin:6px 0}.blank-line{display:inline-block;min-width:62px;border-bottom:2px solid #5d6c80}.match-grid{display:grid;grid-template-columns:1fr 1fr;gap:10px}.match-grid p{margin:3px 0}.response-lines span{display:block;height:21px;border-bottom:1px solid #93a4ba}.watermark-grid{position:absolute;inset:0;display:grid;grid-template-columns:repeat(3,1fr);grid-template-rows:repeat(5,1fr);pointer-events:none;z-index:0}.watermark-grid span{display:grid;place-items:center;transform:rotate(-24deg);font-weight:900;color:rgba(36,87,214,.052);font-size:.72rem}.worksheet-paper>*:not(.watermark-grid){position:relative;z-index:1}.extension-note{margin:6px 0 10px;color:#5d6c80;font-size:.9rem;font-weight:700}.worksheet-paper__head h2{font-size:1.14rem}.reasoning-reminder{margin:0 0 9px;border-left:4px solid #2457d6;background:#eef5ff;padding:7px 9px;border-radius:8px;font-size:.84rem;font-weight:750;color:#173968}.worksheet-model{border:1px solid #cbd9eb;border-radius:10px;background:#f8fbff;padding:6px;margin:0 0 9px;break-inside:avoid}.worksheet-model .y6-board{border:0;background:transparent;margin:0;padding:0}.worksheet-model .y6-board>p{font-size:.76rem}.worksheet-model .y6-table span{padding:4px;font-size:.66rem}.worksheet-model .y6-numberline{padding:10px 6px 25px}.worksheet-model .y6-grid{width:110px}.worksheet-model .y6-coordinate .plot span{min-height:18px}.worksheet-model .y6-orbit{height:145px;transform:scale(.72);transform-origin:center}.worksheet-model .y6-circuit{height:125px;transform:scale(.78);transform-origin:center}.worksheet-model .y6-habitat{height:120px}@media(max-width:650px){.worksheet-shell{padding:10px}.match-grid{grid-template-columns:1fr}.brandbar{display:block}.brandbar small{display:block;margin-top:4px}}@media print{body{background:#fff!important}.worksheet-paper,.worksheet-question,.question-prompt,.worksheet-options span,.match-grid p,.fill-template{color:#203047!important}.worksheet-extension{break-before:page;page-break-before:always}.worksheet-actions,.worksheet-nav,.worksheet-hero{display:none!important}.worksheet-paper+.worksheet-paper{margin-top:0}.worksheet-paper{box-shadow:none!important;background:#fff!important}.worksheet-model{background:#fff!important}}
    `;
  ...
  function evidenceReminder() {
    if (subject === "maths") return "Show a representation or efficient strategy, state units and constraints, and verify using estimation, an inverse, a property or another representation.";
    if (subject === "science") return "Use observations, measurements, models and fair-method reasoning. Separate evidence from explanation and limit conclusions to the conditions tested.";
    return "Use specific language, structural, visual or textual evidence. Explain how the choice shapes meaning for audience and purpose.";
  }
  ...
  function renderScreen() {
    ...
    document.body.innerHTML = `<div class="worksheet-shell"><nav class="worksheet-nav" aria-label="Breadcrumb"><a href="/">Home</a><a href="/year6/curriculum/${subject}/">Year 6 ${subjectName}</a><a href="/year6/${subject}/${unit.slug}/">${code} topic</a></nav><header class="worksheet-hero"><div class="brandbar"><div class="brandmark">SkillrHub <span>F–10</span></div><small>Year 6 ${subjectName} • Student Worksheets</small></div><p class="eyebrow">${code} • Year 6 ${subjectName}</p><h1>${esc(worksheet.title)} Worksheets</h1><p>Worksheet 1 consolidates the core model and reasoning. Related elaboration clusters add an application worksheet and, for broad codes, mixed-mastery work.</p><div class="worksheet-meta"><span>8 core questions</span><span>2 separate enrichment questions</span><span>Additional strand worksheets where needed</span><span>US Letter portrait</span><span>Preview before download</span></div><div class="worksheet-actions"><button class="primary" id="previewCorePdfButton" type="button">Preview Worksheet 1 — Core</button><button class="secondary" id="previewExtensionPdfButton" type="button">Preview enrichment extension</button><a href="/year6/${subject}/${unit.slug}/">Back to topic</a><a href="/quiz/year-6/${routeSubject}/${code.toLowerCase()}/practice/">Open Practice</a></div><div class="worksheet-print-tip">Worksheets may continue to a second page to retain readable text and sufficient response space. Use double-sided printing to save paper.</div></header><main id="worksheetRoot"><section class="worksheet-paper worksheet-core worksheet-core-paper">${watermark()}${paperHeader(`${code} — ${esc(worksheet.title)}`)}<div class="worksheet-model">${unit.model_html}</div><p class="reasoning-reminder">${esc(evidenceReminder())}</p><section class="core-grid">${core.map((question, i) => renderQuestion(question, i)).join("")}</section><footer class="worksheet-footer"><span><strong>SkillrHub F–10</strong> • Year 6 ${subjectName} Core Worksheet</span><span>skillrhub.com</span></footer></section><section class="worksheet-paper worksheet-extension">${watermark()}${paperHeader(`${code} — Enrichment Extension`, "Optional reasoning, investigation and creation challenges")}<p class="extension-note">Use this extension for students ready to compare, generalise, evaluate, investigate or create beyond the core examples.</p><section class="enrichment-grid">${enrichment.map((question, i) => renderQuestion(question, i + core.length)).join("")}</section><footer class="worksheet-footer"><span><strong>SkillrHub F–10</strong> • Optional Enrichment Extension</span><span>skillrhub.com</span></footer></section></main></div>`;
  }
  ...
  async function previewPdf(mode) {
    ...
    try {
      ...
      const questions = extension ? all.filter((q)=>q.enrichment) : all.filter((q)=>!q.enrichment);
      const start = extension ? all.filter((q)=>!q.enrichment).length + 1 : 1;
      ...
      const header = () => {
        ...
        doc.setFont("helvetica","normal"); doc.setFontSize(7.2); doc.text(evidenceReminder(),margin,34,{maxWidth:width});
      ...
      const footer = () => {
        ...
        doc.text(`SkillrHub F-10 • Year 6 ${subjectName} ${extension ? "Enrichment" : "Core"}`,margin,pageH-8);
      ...
      questions.forEach((question,index) => {
        ...
        if (y + estimate > pageH - 18) { footer(); doc.addPage("letter","portrait"); pageNumber++; y=38; header(); }
  ...
  if (meta) meta.content = `${code} Year 6 ${subjectName} worksheets with a visual model, 8 core questions, enrichment and additional connected-strand sheets where needed.`;
  renderScreen();
