(() => {
  ...
  const match = location.pathname.match(/^\/year7\/(maths|science|english)\/(ac9[mse]7[a-z0-9]+)/i);
  if (!match) return;
  ...
  const units = window[`SkillrYear7${subjectName}Data`] || {};
  const order = window[`SkillrYear7${subjectName}Order`] || Object.keys(units);
  ...
  function relatedLinks() {
    return order.map((unitCode) => {
      ...
      return related ? `<li><a href="/year7/${subject}/${related.slug}/">${unitCode}: ${esc(related.title)}</a></li>` : "";
    }).join("");
  }
  ...
  function render() {
    ...
    const international = legacySection("International curriculum mapping") || `<h2>International curriculum mapping</h2><p>The Australian Curriculum code is exact; overseas mappings are broad equivalents.</p>`;
    ...
    document.title = `${code} ${unit.title} | Year 7 ${subjectName}`;
    const description = q('meta[name="description"]');
    if (description) description.content = `Teach ${code} ${unit.desc} with a connected visual Year 7 ${subjectName} lesson, teacher slides, worksheets, Practice and Test.`;

    hero.classList.add("micro-hero");
    hero.innerHTML = `<p class="curriculum-eyebrow">${code} • Year 7 ${subjectName}</p><h1>${esc(unit.title)}</h1><p class="micro-hero__subtitle">${esc(unit.subtitle)}</p><p class="micro-hero__goal">${esc(unit.learn)}</p><div class="hero-visual">${unit.hero_visual}</div><div class="topic-action-row"><a class="primary" href="#teaching-lesson">Start lesson</a><a href="/year7/${subject}/${unit.slug}/teacher-slides/" target="_blank" rel="noopener">Teacher slides</a><a href="/quiz/year-7/${routeSubject}/${code.toLowerCase()}/worksheet/" target="_blank" rel="noopener">Worksheets</a><a href="/quiz/year-7/${routeSubject}/${code.toLowerCase()}/practice/">Practice</a><a href="/quiz/year-7/${routeSubject}/${code.toLowerCase()}/test/">Test</a></div><button class="report-issue-button" type="button" data-report-issue>Report issue</button>`;
    ...
    const reasoning = subject === "maths"
      ...
      : subject === "science"
        ? [["Observe","What can be measured, classified or compared?"],["Explain","How does the model account for evidence?"],["Evaluate","What variables, errors, limits or questions remain?"]]
