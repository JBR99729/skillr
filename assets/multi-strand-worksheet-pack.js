(() => {
  ...
  function openPreview(unit, result, sheetNumber, title, questions) {
    ...
    const sheetLabel = typeof sheetNumber === "string" ? sheetNumber : `Worksheet ${sheetNumber}`;
  ...
  function install(unit, result) {
    ...
    const pairedTopicPractice = document.body.dataset.topicPracticePair === "true";

    const second = document.createElement("button");
    second.type = "button";
    second.className = "secondary worksheet-pack-button";
    second.textContent = pairedTopicPractice ? "Optional Application Extension" : "Preview Worksheet 2 — Application";
    second.addEventListener("click", () => openPreview(unit, result, pairedTopicPractice ? "Optional Application Extension" : 2, "Application and Connections", makeApplicationQuestions(unit, result.clusters)));
    ...
    if (needsThird) {
      ...
      third.textContent = pairedTopicPractice ? "Optional Mixed-Mastery Extension" : "Preview Worksheet 3 — Mixed Mastery";
      third.addEventListener("click", () => openPreview(unit, result, pairedTopicPractice ? "Optional Mixed-Mastery Extension" : 3, "Mixed Mastery and Reasoning", makeMasteryQuestions(unit, result.clusters)));
      actions.appendChild(third);
    }

    const note = document.createElement("p");
    note.className = "worksheet-pack-note";
    note.textContent = pairedTopicPractice ? (needsThird ? "Topic Practice 1 and 2 are the required pair. Optional extensions apply connected strands and mixed mastery." : "Topic Practice 1 and 2 are the required pair. The optional extension applies connected curriculum strands.") : (needsThird ? "Worksheet 1 builds the core concept. Worksheet 2 applies connected strands. Worksheet 3 checks mixed mastery." : "Worksheet 1 builds the core concept. Worksheet 2 applies the connected curriculum strands.");
