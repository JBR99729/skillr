(() => {
  ...
  function ensureCss() {
    ...
    style.textContent = `
      ...
      .a01-topic-hero,.a01-topic-hero *,.a01-topic-layout,.a01-topic-layout *,.a01-slide,.a01-slide *{box-sizing:border-box}
  ...
  function renderTopic(options) {
    ...
    const vocabulary = `<section class="a01-topic-section">${sectionHead("Key Vocabulary", "Say each word, point to the matching model and use it in a sentence.", "Say • show • use")}<div class="a01-at-glance">${spec.vocabulary.map((item) => `<article><h3>${esc(item.term)}</h3><p>${esc(item.definition)}</p></article>`).join("")}</div></section>`;
    ...
    main.innerHTML = `<div class="a01-topic-stack">${atGlance}${vocabulary}${progression}${meaning}${relationships}${samePurpose}${pencil}${opinion}${worked}${misconceptions}${activity}${differentiation}${mastery}${preview}${reference}${resources}</div><aside class="a01-topic-sidebar"><section class="a01-topic-panel"><span class="a01-free-badge">Free Teacher Resource.</span><h2 style="margin-top:10px">Teach directly</h2><p>Open the slides full screen. Every picture, question and model answer is already provided.</p><div class="a01-resource-links"><a class="primary" href="${esc(spec.resourceLinks.slide)}" target="_blank" rel="noopener">Teacher Slides</a></div></section><section class="a01-topic-panel"><h2>Lesson language</h2><p><strong>Same purpose</strong><br>Different listener<br>Different words</p><p>Different languages, dialects and family ways of speaking can all be valid. Choose words for this listener, purpose and place.</p></section></aside>`;

    // The shared page adds international-alignment content after render. Keep
    // that formal reference inside this prototype's optional collapsed panel.
    function absorbInternationalAlignment() {
      const alignment = q("#international-alignment");
      const referenceBody = q(".a01-reference__body");
      if (!alignment || !referenceBody || referenceBody.querySelector("[data-a01-international]")) return;
      const wrapper = document.createElement("div");
      wrapper.dataset.a01International = "true";
      wrapper.innerHTML = alignment.innerHTML;
      referenceBody.appendChild(wrapper);
      alignment.remove();
    }
    [0, 250, 1000, 2500].forEach((delay) => window.setTimeout(absorbInternationalAlignment, delay));
  ...
  function slidePattern(slide) {
    const learningIntent = slide.coreRole === "learning-intention" ? `<div class="a01-purpose-banner"><span>We are learning to ${esc(spec.learningIntention.replace(/^(?:I can|To)\s+/i, "").replace(/\.$/, ""))}.</span><span>I can ${esc(spec.successCriteria[0].replace(/^I can\s+/i, ""))}</span></div>` : "";
    return `<div class="a01-pattern">${learningIntent}<div class="a01-pattern__meaning"><span class="a01-pattern-label">What it means</span><span>${esc(slide.display.meaning)}</span></div><div class="a01-pattern__picture"><span class="a01-pattern-label">Look at the picture</span>${slide.display.modelIds.map((id) => modelHtml(id, { showPurpose: false })).join("")}</div><div class="a01-pattern__bottom"><div class="a01-slide-ask"><span class="a01-pattern-label">Ask the class</span><strong>${esc(slide.display.studentPrompt)}</strong></div><div class="a01-slide-check"><button class="a01-check-button" type="button" data-reveal-answer aria-expanded="false">Check together</button><div class="a01-check-answer" data-check-answer hidden>${esc(slide.display.checkTogether)}</div></div></div></div>`;
  }
  ...
  function renderSlides(options) {
    ...
    document.body.innerHTML = `<div class="a01-slide-app"><nav class="a01-toolbar" aria-label="Teacher slide controls"><div class="a01-toolbar__group"><a href="${esc(spec.resourceLinks.topic)}">Back to Topic Guide</a><button type="button" data-slide-prev aria-label="Previous slide">Previous</button><button type="button" data-slide-next aria-label="Next slide">Next</button></div><div class="a01-toolbar__group"><label class="a01-slide-count" for="a01-slide-select">Slide <span data-slide-number>1</span> of ${spec.slides.length}</label><select id="a01-slide-select" aria-label="Choose a slide">${spec.slides.map((slide, index) => `<option value="${index}">${slide.sequenceRole === "core" ? "Core" : "Optional"} ${index + 1}. ${esc(slide.title)}</option>`).join("")}</select><button type="button" data-slide-fullscreen>Full screen</button></div></nav><div class="a01-stage-wrap"><main class="a01-stage" aria-live="polite">${spec.slides.map((slide, index) => `<section class="a01-slide" data-slide-index="${index}"${index ? " hidden" : ""}><div class="a01-slide__watermark" aria-hidden="true">SkillrHub</div><div class="a01-slide__url-watermark" aria-hidden="true">https://skillrhub.com</div><header class="a01-slide-header"><h1>${esc(slide.title)}</h1><span class="a01-free-badge">${slide.sequenceRole === "core" ? "Core slide" : "Optional extension"}</span></header>${slidePattern(slide)}<footer class="a01-slide-footer"><div class="a01-slide-footer__brand"><strong>SkillrHub</strong><span>skillrhub.com</span><span>© 2026</span><span>Free classroom use</span><span>${CODE}</span></div><span>${index + 1} / ${spec.slides.length}</span></footer></section>`).join("")}</main></div><aside class="a01-notes" data-slide-notes>${notesHtml(spec.slides[0])}</aside></div>`;
