(() => {
  ...
  const match = location.pathname.match(/^\/quiz\/year-2\/science\/(ac9s2[a-z0-9]+)\/worksheet\/?$/i);
  if (!match) return;
  ...
  function renderScreen(unit, worksheet) {
    ...
    document.body.innerHTML = `<div class="worksheet-shell"><nav class="worksheet-nav" aria-label="Breadcrumb"><a href="/">Home</a><a href="/year2/curriculum/science/">Year 2 Science</a><a href="/year2/science/${unit.slug}/">${code} topic</a></nav><header class="worksheet-hero"><div class="brandbar"><div class="brandmark">SkillrHub <span>F–10</span></div><small>Year 2 Science • Student Worksheet</small></div><p class="eyebrow">${code} • Year 2 Science</p><h1>${esc(worksheet.title)} Worksheet</h1><div class="worksheet-meta"><span>Core sheet: 8 questions</span><span>Separate enrichment: 2 questions</span><span>US Letter portrait</span><span>Preview before download</span></div><div class="worksheet-actions"><button class="primary" id="previewCorePdfButton" type="button">Preview core worksheet</button><button class="secondary" id="previewExtensionPdfButton" type="button">Preview enrichment extension</button><a href="/year2/science/${unit.slug}/">Back to topic</a><a href="/quiz/year-2/science/${code.toLowerCase()}/practice/">Open practice</a></div><div class="worksheet-print-tip">Print the core sheet for the class. Print the enrichment extension only when needed.</div></header><main id="worksheetRoot"><section class="worksheet-paper worksheet-core">${watermark()}${paperHeader(`${code} — ${esc(worksheet.title)}`)}<section class="core-grid">${core.map((question, i) => renderQuestion(question, i)).join("")}</section><footer class="worksheet-footer"><span><strong>SkillrHub F–10</strong> • Year 2 Science Core Worksheet</span><span>skillrhub.com</span></footer></section><section class="worksheet-paper worksheet-extension">${watermark()}${paperHeader(`${code} — Enrichment Extension`, "Optional challenge questions")}<p class="extension-note">Use this page for students ready to explain, justify, design or evaluate using the same lesson idea.</p><section class="enrichment-grid">${enrichment.map((question, i) => renderQuestion(question, i + core.length)).join("")}</section><footer class="worksheet-footer"><span><strong>SkillrHub F–10</strong> • Optional Enrichment Extension</span><span>skillrhub.com</span></footer></section></main></div>`;
  }
  ...
  async function previewPdf(worksheet, mode) {
    ...
    try {
      ...
      const footer = () => {
        ...
        doc.setFont("helvetica", "bold"); doc.setFontSize(8); doc.setTextColor(36,87,214); doc.text(`SkillrHub F-10 • Year 2 Science ${isExtension ? "Enrichment" : "Core"}`, margin, pageH-8); doc.text("skillrhub.com", pageW-margin, pageH-8, { align: "right" });
      };
      header();
  ...
  async function init() {
    try {
      if (!window.SkillrYear2ScienceData?.[code]) await loadScript("/assets/year2-science-data.js?v=1");
      ensureCss();
      const unit = window.SkillrYear2ScienceData?.[code];
      const worksheet = window.SkillrYear2ScienceWorksheetData?.[code];
      if (!unit || !worksheet) return;
      document.title = `${code} ${worksheet.title} Worksheet | SkillrHub`;
      const meta = document.querySelector('meta[name="description"]');
      if (meta) meta.content = `${code} Year 2 Science worksheet with an 8-question core sheet and separate 2-question enrichment extension.`;
