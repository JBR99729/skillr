(() => {
  ...
  const order = window.SkillrYear4MathsOrder || Object.keys(units);
  const match = location.pathname.match(/\/year4\/maths\/(ac9m4[a-z0-9]+)/i);
  ...
  function relatedLinks() {
    return order.map((unitCode) => {
      ...
      return related ? `<li><a href="/year4/maths/${related.slug}/">${unitCode}: ${esc(related.title)}</a></li>` : "";
    }).join("");
  }
  ...
  function render() {
    ...
    document.title = `${code} ${unit.title} | Year 4 Maths`;
    const description = q('meta[name="description"]');
    if (description) description.content = `Teach ${code} ${unit.desc} with a visual Year 4 Maths lesson, worked reasoning, live teacher slide, authored worksheet, Practice and Test.`;

    hero.classList.add("micro-hero");
    hero.innerHTML = `<p class="curriculum-eyebrow">${code} • Year 4 Maths</p><h1>${esc(unit.title)}</h1><p class="micro-hero__subtitle">${esc(unit.subtitle)}</p><p class="micro-hero__goal">${esc(unit.learn)}</p><div class="hero-visual">${unit.hero_visual}</div><div class="topic-action-row"><a class="primary" href="#teaching-lesson">Start lesson</a><a href="/year4/maths/${unit.slug}/teacher-slides/" target="_blank" rel="noopener">Teacher slide</a><a href="/quiz/year-4/math/${code.toLowerCase()}/worksheet/" target="_blank" rel="noopener">Worksheet</a><a href="/quiz/year-4/math/${code.toLowerCase()}/practice/">Practice</a><a href="/quiz/year-4/math/${code.toLowerCase()}/test/">Test</a></div><button class="report-issue-button" type="button" data-report-issue>Report issue</button>`;
    ...
    const successItems = unit.mastery.slice(0, 4).map((item) => `<li>${esc(item)}</li>`).join("");

    const lesson = `<details class="topic-menu combined-lesson-menu" id="teaching-lesson" open><summary><span class="menu-title">Teaching Lesson: Model → Reason → Apply → Justify</span></summary><div class="menu-content combined-lesson-content"><section class="lesson-part"><h3>Curriculum focus and lesson scope</h3><div class="curriculum-focus"><strong>${code}:</strong><p>${esc(unit.desc)}</p><div class="lesson-timing"><span>Teacher model: 10–15 min</span><span>Guided reasoning: 8–10 min</span><span>Activity: 10–15 min</span><span>Independent check: 5–10 min</span></div></div></section><section class="lesson-part"><h3>Learning intention and success criteria</h3><div class="success-grid"><div class="success-card"><h4>Learning intention</h4><p>${esc(unit.learn)}</p></div><div class="success-card"><h4>I can…</h4><ul>${successItems}</ul></div></div></section><section class="lesson-part"><h3>Reasoning routine</h3><div class="lesson-callout">${esc(unit.routine)}</div></section><section class="lesson-part"><h3>Teach It</h3><p><strong>${esc(unit.model_title)}</strong></p>${unit.model_html}<div class="reasoning-strip"><div><strong>Notice</strong><span>What quantities, structures or relationships are visible?</span></div><div><strong>Explain</strong><span>Why does the model work, not just what answer it gives?</span></div><div><strong>Check</strong><span>What estimate, inverse, equivalent form or counterexample verifies it?</span></div></div></section><section class="lesson-part"><h3>Apply and Transfer</h3><p><strong>${esc(unit.apply_title)}</strong></p>${unit.apply_html}</section><section class="lesson-part"><h3>Guided and collaborative activities</h3><div class="mini-grid-3">${activities}</div></section><section class="lesson-part"><h3>Common Mix-Ups</h3><div class="mini-grid-4">${mixups}</div></section><section class="lesson-part"><h3>Quick Check</h3><div class="mini-grid-4">${checks}</div></section></div></details>`;

    main.className = "curriculum-layout lesson-layout";
    main.innerHTML = `<div class="lesson-stack">${lesson}<details class="topic-menu"><summary><span class="menu-title">Quick Mastery Check</span></summary><div class="menu-content"><p><strong>Move to Practice or Test when the student can complete these independently and explain their reasoning.</strong></p><div class="mastery-row">${mastery}</div></div></details>${menu("Australian Curriculum description & elaborations","Reference",curriculum)}${menu("Teacher resource","Resource",`<div class="teacher-resource-copy"><h3>${code} live classroom teaching slide</h3><p>Project the condensed lesson with the same model, reasoning prompts, application example and visual language used on this topic page.</p><a class="curriculum-button primary" href="/year4/maths/${unit.slug}/teacher-slides/" target="_blank" rel="noopener">Open teacher slide</a></div>`,"teacher-slide")}${menu("International curriculum mapping","Links",international)}${menu("Related Year 4 Maths topics","Links",`<ul class="curriculum-related-list">${relatedLinks()}</ul>`)}${menu("Official references","Sources",official)}<section class="lesson-part" id="skillr-expansion-note"><div class="curriculum-focus"><strong>Need more detail on this topic?</strong><p>Email <a href="mailto:skillrhublearning@gmail.com">skillrhublearning@gmail.com</a> with the part you would like explained further. SkillrHub will use that feedback to expand this section.</p></div></section></div><aside class="curriculum-sidebar"><section class="curriculum-panel"><h2>Next step</h2><div class="curriculum-link-row"><a class="curriculum-button primary" href="/quiz/year-4/math/${code.toLowerCase()}/practice/">Practice</a><a class="curriculum-button" href="/quiz/year-4/math/${code.toLowerCase()}/worksheet/" target="_blank" rel="noopener">Worksheet</a><a class="curriculum-button" href="/quiz/year-4/math/${code.toLowerCase()}/test/">Test</a></div></section><section class="curriculum-panel"><h2>Learning path</h2><div class="curriculum-link-row"><a href="/year3/">Previous year</a><a href="/year5/">Next year</a><a href="/year4/curriculum/maths/">All Year 4 Maths</a></div></section></aside>`;
