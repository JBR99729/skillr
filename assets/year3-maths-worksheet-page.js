(() => {
  ...
  const match = location.pathname.match(/^\/quiz\/year-3\/math\/(ac9m3[a-z0-9]+)\/worksheet(?:\/(topic-practice-[12]))?\/?$/i);
  if (!match) return;
  const code = match[1].toUpperCase();
  const practiceSlug = match[2] || "topic-practice-1";
  const practiceNumber = practiceSlug === "topic-practice-2" ? 2 : 1;
  ...
  function ensureCss() {
    ...
    style.textContent = `
      body{background:#f4f7fb}.worksheet-shell{max-width:1060px;margin:0 auto;padding:18px}.worksheet-nav{display:flex;flex-wrap:wrap;gap:8px;margin-bottom:12px}.worksheet-nav a{color:#2457d6;font-weight:800;text-decoration:none}.worksheet-hero{border:1px solid #d8e2ef;border-radius:18px;background:#fff;padding:20px;box-shadow:0 8px 24px rgba(28,55,91,.08)}.worksheet-hero h1{font-size:clamp(1.45rem,3vw,2rem);margin:.25rem 0}.worksheet-meta,.worksheet-tabs{display:flex;flex-wrap:wrap;gap:8px;margin:12px 0}.worksheet-meta span{border:1px solid #d7e3fb;background:#f7faff;border-radius:999px;padding:6px 9px;font-size:.82rem;font-weight:800;color:#173968}.worksheet-tabs a{border:2px solid #2457d6;border-radius:10px;padding:9px 14px;font-weight:900;text-decoration:none;color:#2457d6}.worksheet-tabs a[aria-current="page"]{background:#2457d6;color:#fff}.worksheet-actions{display:flex;flex-wrap:wrap;gap:8px;margin-top:12px}.worksheet-actions a,.worksheet-actions button{border:0;border-radius:12px;padding:10px 14px;font-weight:900;text-decoration:none;cursor:pointer}.worksheet-actions .primary{background:#2457d6;color:#fff}.worksheet-actions a{background:#edf2f7;color:#173968}.worksheet-print-tip{margin-top:10px;color:#5d6c80;font-size:.9rem}.brandbar{display:flex;justify-content:space-between;gap:12px;align-items:center}.brandmark{display:flex;align-items:center;gap:8px;font-weight:900;color:#2457d6}.brandmark img{width:38px;height:38px}.brandmark span{color:#173968}.worksheet-paper,.worksheet-question,.question-prompt,.worksheet-options span,.match-grid p{color:#203047}.worksheet-paper{position:relative;overflow-wrap:anywhere;background:#fff;border:1.5px solid #2457d6!important}.worksheet-question{break-inside:avoid;page-break-inside:avoid}.tier-heading{margin:18px 0 8px;color:#173968;border-bottom:2px solid #d7e3fb;break-after:avoid;page-break-after:avoid}.worksheet-question .question-line{display:flex;align-items:flex-start;gap:6px}.worksheet-question .question-prompt{margin:0;flex:1}.question-number-text{font-weight:900;color:#173968}.worksheet-options{display:grid!important;grid-auto-flow:column;grid-auto-columns:minmax(0,1fr);gap:8px 12px;margin:7px 0}.worksheet-options span{white-space:normal;min-width:0}.fill-template{font-weight:800;color:#173968;margin:6px 0}.blank-line{display:inline-block;min-width:58px;border-bottom:2px solid #5d6c80}.match-grid{display:grid;grid-template-columns:1fr 1fr;gap:10px}.match-grid p{margin:3px 0}.response-lines span{display:block;height:18px;border-bottom:1px solid #93a4ba}.watermark-grid{position:absolute;inset:0;display:grid;grid-template-columns:repeat(3,1fr);grid-template-rows:repeat(5,1fr);pointer-events:none;z-index:0}.watermark-grid span{display:grid;place-items:center;transform:rotate(-24deg);font-weight:900;color:rgba(36,87,214,.05);font-size:.72rem}.worksheet-paper>*:not(.watermark-grid){position:relative;z-index:1}.answer-key{break-before:page;page-break-before:always}.answer-key li{margin-bottom:10px;break-inside:avoid;page-break-inside:avoid}.answer-key p{margin:2px 0}.worksheet-paper__head h2{font-size:1.14rem}@media(max-width:650px){.worksheet-shell{padding:10px}.match-grid{grid-template-columns:1fr}.worksheet-options{display:flex!important;flex-wrap:wrap}.worksheet-options span{flex:1 1 45%}.brandbar{display:block}.brandbar small{display:block;margin-top:4px}}@page{size:A4 portrait;margin:12mm}@media print{html,body{width:210mm}body{background:#fff!important;font-size:10.5pt}.worksheet-actions,.worksheet-nav,.worksheet-hero{display:none!important}.worksheet-paper{box-shadow:none!important;background:#fff!important;max-width:186mm;margin:0 auto;border-width:1px!important}.worksheet-options{display:grid!important;grid-auto-flow:column;grid-auto-columns:minmax(0,1fr);gap:4mm}.match-grid{max-width:100%}.brandmark img{max-width:12mm;max-height:12mm}}
    `;
  ...
  function selectSheetQuestions(worksheet) {
    const all = worksheet.questions.map((question,index)=>({question,index}));
    const targetSheetId = `${code.toLowerCase()}-topic-practice-${practiceNumber}`;
    const authored = all.filter(({question}) => question.sheet_id === targetSheetId);
    if (authored.length) return {all, selected: authored};
    return {all, selected: practiceNumber===1 ? all.slice(0,5) : all.slice(5)};
  }

  function renderScreen(unit, worksheet) {
    const {all, selected}=selectSheetQuestions(worksheet);
    const tiers=[["warm-up","Tier 1: Warm-Up"],["core","Tier 2: Core Practice"],["extension","Tier 3: Extension / Challenge"]];
    const questions=tiers.map(([key,label])=>{const items=selected.filter(x=>x.question.tier===key);return items.length?`<h2 class="tier-heading">${label}</h2>${items.map(x=>renderQuestion(x.question,x.index)).join("")}`:""}).join("");
    const answers=selected.map(({question:q,index})=>`<li><strong>${index+1}. ${esc(q.answer)}</strong><p>Summary: ${esc(q.summary)}</p><p>Hint: ${esc(q.hint)}</p></li>`).join("");
    const base=`/quiz/year-3/math/${code.toLowerCase()}/worksheet`;
    const title=`Topic Practice ${practiceNumber}`;
    const retainedLessonSupport = document.getElementById("lesson-support")?.outerHTML || "";
    const retainedLessonPractice = document.getElementById("lesson-practice")?.outerHTML || "";
    document.body.innerHTML = `${retainedLessonSupport}${retainedLessonPractice}<div class="worksheet-shell"><nav class="worksheet-nav" aria-label="Breadcrumb"><a href="/">Home</a><a href="/year3/curriculum/maths/">Year 3 Maths</a><a href="/year3/maths/${unit.slug}/">${code} topic</a></nav><header class="worksheet-hero"><div class="brandbar"><div class="brandmark"><img src="/icons/icon-192.png" alt="SkillrHub logo">SkillrHub <span>F–10</span></div><small>Year 3 Maths • ${title}</small></div><p class="eyebrow">${code} • Year 3 Maths</p><h1>${title}</h1><p>${selected.length} questions in this sheet. Together the two sheets cover ${all.length} aligned questions without repeats.</p><nav class="worksheet-tabs" aria-label="Topic practice sheets"><a href="${base}/topic-practice-1/" ${practiceNumber===1?'aria-current="page"':""}>Topic Practice 1</a><a href="${base}/topic-practice-2/" ${practiceNumber===2?'aria-current="page"':""}>Topic Practice 2</a></nav><div class="worksheet-meta"><span>${selected.length} questions</span><span>${selected.filter(x=>x.question.tier==="warm-up").length} Warm-Up</span><span>${selected.filter(x=>x.question.tier==="core").length} Core</span><span>${selected.filter(x=>x.question.tier==="extension").length} Challenge</span><span>Own answer key</span></div><div class="worksheet-actions"><button class="primary" onclick="window.print()" type="button">Print ${title}</button><a href="/year3/maths/${unit.slug}/">Back to topic</a><a href="/year3/maths/${unit.slug}/teacher-slides/">Teacher slides</a><a href="/quiz/year-3/math/${code.toLowerCase()}/practice/">Open Practice</a><a href="/quiz/year-3/math/${code.toLowerCase()}/test/">Open Test</a></div></header><main id="worksheetRoot"><section class="worksheet-paper">${watermark()}${paperHeader(`${code} — ${title}`)}${questions}<footer class="worksheet-footer"><span><strong>SkillrHub F–10</strong> • ${title}</span><span>skillrhub.com</span></footer></section><section class="worksheet-paper answer-key">${paperHeader(`${code} — ${title} Answer Key`,"Only questions from this sheet")}<ol>${answers}</ol></section></main></div>`;
  }
  ...
  async function init() {
    try {
      if (!window.SkillrYear3MathsRegister) await loadScript("/assets/year3-maths-data-base.js?v=4");
      const files = ["/assets/year3-maths-data-n1.js?v=4","/assets/year3-maths-data-n2.js?v=4","/assets/year3-maths-data-n3.js?v=4","/assets/year3-maths-data-a.js?v=4","/assets/year3-maths-data-m1.js?v=4","/assets/year3-maths-data-m2.js?v=4","/assets/year3-maths-data-sp.js?v=4","/assets/year3-maths-data-st.js?v=4","/assets/year3-maths-data-p.js?v=4"];
      for (const file of files) { if (!window.SkillrYear3MathsData?.[code]) await loadScript(file); }
      if (code === "AC9M3M04") await loadScript("/assets/year3-maths-worksheet-authored-overrides.js?v=20260823-ac9m3m04-v1");
      ensureCss();
      const unit=window.SkillrYear3MathsData?.[code], worksheet=window.SkillrYear3MathsWorksheetData?.[code];
      if(!unit||!worksheet)return;
      document.title=`${code} Topic Practice ${practiceNumber} | SkillrHub`;
      const meta=document.querySelector('meta[name="description"]'); if(meta)meta.content=`${code} Year 3 Maths Topic Practice ${practiceNumber}, an authored printable practice sheet with its own checked answer key.`;
