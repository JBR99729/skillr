(() => {
  ...
  function isCompletedResource() {
    ...
    if (/^\/(year1|quiz\/year-1|worksheets\/year1)\//.test(path)) return true;
    if (/^\/(year2|quiz\/year-2|worksheets\/year2)\//.test(path)) return true;
    if (/^\/(year3|quiz\/year-3|worksheets\/year3)\//.test(path)) return true;
