(() => {
  ...
  const defaults = {maths:"AC9M7N01",science:"AC9S7U01",english:"AC9E7LA01"};
  ...
  if (back) back.href = `/year7/${subject}/${unit.slug}/`;
  ...
  const reasoning = subject === "maths"
    ? [["Represent","Identify quantities, structure and constraints."],["Reason","Explain why the property or strategy works."],["Verify","Use an estimate, inverse or alternate representation."]]
    : subject === "science"
  ...
  root.innerHTML = `<div class="skillr-repeat-watermark" aria-hidden="true">${Array.from({length:15},()=>"<span>SkillrHub F–10 • skillrhub.com</span>").join("")}</div><div class="brandbar"><div class="brand">SkillrHub <span>F–10</span></div><small>Year 7 ${subjectName} • Live classroom display</small></div><section class="hero"><div><p class="eyebrow">${code} • Year 7 ${subjectName}</p><h1>${esc(unit.title)}</h1><p class="subtitle">${esc(unit.subtitle)}</p><p class="goal"><strong>Learning goal:</strong> ${esc(unit.learn)}</p></div><div class="hero-box"><p><strong>Curriculum focus</strong></p><p>${esc(unit.desc)}</p><p><strong>Learning routine</strong></p><p>${esc(unit.routine)}</p></div></section><div class="flow-row">${routine}</div><section class="model"><div class="model-grid"><div><span class="tag">Teach It</span><h2>${esc(unit.model_title)}</h2>${unit.model_html}</div><div><span class="tag">Apply and transfer</span><h2>${esc(unit.apply_title)}</h2>${unit.apply_html}</div></div></section><section class="reasoning">${reasoning.map(([title,text])=>`<div><strong>${title}</strong>${esc(text)}</div>`).join("")}</section><section class="grid"><div class="card"><span class="tag">Try it</span><h2>Activities</h2><ul>${activities}</ul></div><div class="card mistake"><span class="tag">Common Mix-Ups</span><h2>Watch for</h2><ul>${mixups}</ul></div><div class="card check"><span class="tag">Quick check</span><h2>Ask</h2><ol>${checks}</ol></div></section><div class="footer"><span>Project directly in class • same models as the topic page</span><span>skillrhub.com • ${code}</span></div>`;
})();
