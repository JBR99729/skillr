(() => {
  ...
  function renderModel(spec, unit, id) {
    ...
    if (!model) return "";
    const flow = model.component === "curriculumContextFlow"
      ...
      : "";
    const html = model.parameters?.displayHtml || flow;
    const photos = model.component === "foundationScienceEvidence" ? renderPhotos(unit) : "";
    return `<div class="v11-model" data-model-id="${esc(model.id)}"><div class="v11-model__title">${esc(model.purpose)}</div><div class="v11-model__body">${photos}${html || `<p>${esc(model.accessibleDescription)}</p>`}</div></div>`;
  }
  ...
  function relatedLinks(data, order, config) {
    return order.map((code) => data[code]).filter(Boolean).map((unit, index) => `<li><a href="/foundation/${esc(config.pathSegment)}/${esc(unit.slug)}/">${esc(order[index])}: ${esc(unit.title)}</a></li>`).join("");
  }
  ...
  function renderTopic({ data, order, config }) {
    ...
    const application = `<section class="v11-section"><div class="v11-section-head"><h2>Worked application</h2><span class="v11-badge">Model → apply</span></div>${renderModel(spec, unit, "application-model")}</section>`;
    ...
    main.innerHTML = `<div class="v11-stack"><details class="v11-panel v11-panel--lesson" id="teaching-lesson" open><summary>Teach the complete curriculum code</summary><div class="v11-panel__body v11-lesson">${glance}${progression}${mainModel}${application}${elaborations}${misconceptions}${activity}${mastery}${preview}</div></details><details class="v11-panel"><summary>Australian Curriculum description and elaborations</summary><div class="v11-panel__body">${referenceHtml(spec)}</div></details><details class="v11-panel" id="teacher-slide"><summary>Teacher resource</summary><div class="v11-panel__body"><h3>Selectable classroom slides</h3><p>One teaching purpose per 16:9 slide, with teacher actions and checkpoints available below the projector view.</p><div class="v11-resource-row"><a class="primary" href="${esc(spec.resourceLinks.slide)}" target="_blank" rel="noopener">Open Teacher Slides</a></div></div></details><details class="v11-panel"><summary>International curriculum mapping</summary><div class="v11-panel__body">${international || "<p>Use the Australian Curriculum code as the exact reference and map it to the closest local Foundation or Kindergarten outcome.</p>"}</div></details><details class="v11-panel"><summary>Related Foundation ${esc(spec.subject)} topics</summary><div class="v11-panel__body"><ul class="v11-related">${relatedLinks(data, order, config)}</ul></div></details><details class="v11-panel"><summary>Official references</summary><div class="v11-panel__body">${official || '<p><a href="https://www.australiancurriculum.edu.au/" target="_blank" rel="nofollow noopener">Australian Curriculum Version 9.0</a></p>'}</div></details></div><aside class="curriculum-sidebar"><section class="curriculum-panel"><h2>Next step</h2><div class="curriculum-link-row"><a class="curriculum-button primary" href="${esc(spec.resourceLinks.practice)}">Practice</a><a class="curriculum-button" href="${esc(spec.resourceLinks.worksheet)}" target="_blank" rel="noopener">Practice Sheet</a><a class="curriculum-button" href="${esc(spec.resourceLinks.test)}">Test</a></div></section><section class="curriculum-panel"><h2>Learning path</h2><div class="curriculum-link-row">${next ? `<a href="/foundation/${esc(config.pathSegment)}/${esc(data[next].slug)}/">Next ${esc(spec.subject)} unit</a>` : ""}${previous ? `<a href="/foundation/${esc(config.pathSegment)}/${esc(data[previous].slug)}/">Previous ${esc(spec.subject)} unit</a>` : ""}<a href="/year1/">Next year</a></div></section></aside>`;
