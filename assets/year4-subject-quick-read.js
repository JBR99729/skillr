(() => {
  ...
  if (!match) return;
  if (match[1] === "science" && /^ac9s4(?:u0[1-4]|h0[12]|i0[1-6])$/i.test(match[2]) && document.querySelector('[data-skillr-authored-preparation="true"]')) return;
  if (match[1].toLowerCase() === "english" && /^ac9e4(?:la(?:0[1-9]|1[0-2])|le0[1-5]|ly01)$/i.test(match[2]) && document.querySelector('[data-skillr-authored-preparation="true"]')) return;
