(() => {
  ...
  const match = location.pathname.match(/^\/quiz\/year-1\/science\/(ac9s1[a-z0-9]+)\/(practice|test)\/?$/i);
  if (!match) return;
  ...
  function loadData() {
    return new Promise((resolve, reject) => {
      if (window.SkillrYear1ScienceData?.[code]) return resolve();
      const existing = [...document.scripts].find((script) => script.src.includes("/assets/year1-science-data.js"));
      if (existing) {
      ...
      script.src = "/assets/year1-science-data.js?v=1";
      script.async = false;
  ...
  function ensureStyle() {
    if (document.getElementById("skillr-year1-science-quick-read-style")) return;
    const style = document.createElement("style");
    style.id = "skillr-year1-science-quick-read-style";
    style.textContent = `
      #startScreen .start-card{max-width:880px;padding:24px 26px;text-align:left}
      ...
      .year1-science-quick-cards{display:flex;flex-wrap:wrap;gap:7px;margin:0 0 8px}
      .year1-science-quick-cards span{display:inline-flex;align-items:center;justify-content:center;min-height:34px;border:1px solid #d9e5f5;border-radius:999px;background:#fff;padding:7px 9px;color:#173968;font-size:.74rem;font-weight:850}
  ...
  function apply() {
    const unit = window.SkillrYear1ScienceData?.[code];
    ...
    if (intro) intro.textContent = mode === "test" ? "Review the key science notes, then take the Test." : "Review the key science notes, then start Practice.";
    ...
    if (notes.dataset.skillrYear1ScienceQuickRead === `${code}-${mode}`) return true;
    ...
    notes.innerHTML = `<h2>60-second Quick Read</h2><div class="year1-science-quick-cards">${cards}</div><ul>${items.map((item) => `<li>${item}</li>`).join("")}</ul>`;
    notes.dataset.skillrTopicSynced = "true";
    notes.dataset.skillrYear1ScienceQuickRead = `${code}-${mode}`;
    return true;
  }
  ...
  loadData()
    ...
    .catch((error) => console.error("SkillrHub Year 1 Science Quick Read failed:", error));
})();