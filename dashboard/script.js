(function () {
  ...
  async function printProgressCertificate() {
    ...
    try {
      ...
      const correct = attempts.reduce((sum, item) => sum + (Number(item.score) || 0), 0);
      const checkedQuestions = attempts.reduce((sum,item) => sum + Math.max(0, (Number(item.total)||0) - (Number(item.pendingReview)||0)),0);
      const accuracy = checkedQuestions ? Math.round(correct / checkedQuestions * 100) : 0;
      ...
      const recentMarkup = recent.length
        ? recent.map((item) => `<li><span>${escapeHtml(activityTitle(item))}</span><strong>${item.pendingReview ? `${item.score}/${item.markedTotal} checked; ${item.pendingReview} awaiting review` : `${Number(item.percentage) || 0}%`}</strong></li>`).join("")
      ...
      </style></head><body><section class="sheet"><header class="header"><div class="brand"><span class="mark">S</span><span>SkillrHub<small>Learn &amp; Grow</small></span></div><div class="date">Presented on<br><strong>${issuedDate}</strong></div></header><main class="main"><p class="eyebrow">Certificate of Learning Progress</p><h1>This certificate recognises</h1><h2>${escapeHtml(state.profile.name || "Learner")}</h2><p class="recognition">for committed learning and continued growth with SkillrHub.</p></main><section class="metrics"><div class="metric"><strong>${activeTime}</strong><span>Active learning time</span></div><div class="metric"><strong>${questions}</strong><span>Questions</span></div><div class="metric"><strong>${accuracy}%</strong><span>Accuracy of checked answers</span></div><div class="metric"><strong>${skills.size}</strong><span>Skills covered</span></div><div class="metric"><strong>${tests}</strong><span>Tests completed</span></div></section><section class="details"><div class="learning"><h3>Recent learning</h3><ul>${recentMarkup}</ul></div><div class="share">${qrDataUrl ? `<img src="${qrDataUrl}" alt="QR code to SkillrHub">` : ""}<div><strong>Share free learning</strong><span>Scan to visit skillrhub.com. Share SkillrHub with families and teachers looking for free Australian Curriculum learning resources.</span></div></div></section><footer class="footer"><span><strong>SkillrHub</strong> - Free F-10 Australian Curriculum learning</span><span>skillrhub.com - No learner login required</span></footer></section><p class="screen-note">Choose Print or Save as PDF in the print dialog.</p></body></html>`);
      const certificateDocument = certificateWindow.document;
      const formatStyle = certificateDocument.createElement("style");
      formatStyle.textContent = `@page{size:Letter portrait;margin:.35in}html,body{background:#fff!important;font-family:Arial,sans-serif!important}.sheet{width:7.8in!important;min-height:9.75in!important;margin:0 auto!important;padding:.48in .55in .36in!important;display:flex!important;flex-direction:column!important;border:8px solid #1a3a72!important;box-shadow:none!important}.sheet:before{display:none!important}.header{justify-content:center!important;gap:.14in!important;border:0!important;padding:0!important}.certificate-mark{width:.82in;height:.82in}.mark-book{fill:#edf3ff;stroke:#1a3a72;stroke-width:4;stroke-linejoin:round}.mark-fold,.mark-s,.mark-h{fill:none;stroke:#1a3a72;stroke-width:4;stroke-linecap:round;stroke-linejoin:round}.mark-s,.mark-h{stroke:#d68a00}.header .brand{display:block!important;margin:0!important;color:#1a3a72!important;font-size:18px!important;letter-spacing:.08em!important;text-align:left!important;text-transform:uppercase}.header .brand small{font-size:11px!important;letter-spacing:.12em!important}.date{margin:.08in 0!important;text-align:center!important}.main{padding:.1in 0!important}.metrics{gap:.07in!important;margin:.12in 0!important}.metric{padding:.1in .04in!important}.metric strong{font-size:18px!important}.details{grid-template-columns:1fr!important;gap:.1in!important}.learning{padding:.1in .14in!important}.learning li{padding:.05in 0!important;font-size:10px!important}.share{grid-template-columns:.65in 1fr!important;padding:.08in!important}.share img{width:.65in!important;height:.65in!important}.certificate-footer{display:grid;grid-template-columns:repeat(4,1fr);gap:.08in;margin-top:auto;padding-top:.18in;border-top:1px solid #aab7ca;color:#1a3a72}.footer-item{display:flex;align-items:center;justify-content:center;gap:6px;font-size:9px;font-weight:700}.footer-item svg{width:17px;height:17px;fill:none;stroke:currentColor;stroke-width:1.8}@media print{html,body{width:7.8in!important;height:10.3in!important}.sheet{width:7.8in!important;min-height:9.75in!important;max-height:10.3in!important}}`;
      certificateDocument.head.appendChild(formatStyle);
      const certificateHeader = certificateDocument.querySelector(".header");
      if (certificateHeader) certificateHeader.innerHTML = `${certificateBrandMark}<p class="brand">SkillrHub<small>Learn &amp; Grow</small></p>`;
      certificateDocument.querySelector(".footer")?.remove();
      certificateDocument.querySelector(".sheet")?.insertAdjacentHTML("beforeend", certificateFooter);
