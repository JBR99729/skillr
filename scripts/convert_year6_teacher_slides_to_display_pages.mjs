...
const baseDirs = ["year6/english", "year6/maths", "year6/science"];
...
const findSection = (html, label) => {
  const detailsMarker = new RegExp(`<details[^>]*>\\s*<summary>\\s*(?:<strong>)?${label}(?:<\\/strong>)?\\s*<\\/summary>\\s*<div class="(?:curriculum-detail-body|static-topic-panel)">`, "i");
...
const extractExamples = (topicHtml) => {
  let sections = [
    findSectionAny(topicHtml, ["Key concept and central model", "Topic Guide", "Concept models and worked thinking", "Concept model and worked thinking", "Worked and modelled examples", "Concept teaching", "Concept model and worked application", "2\\. Subject-Specific Content &amp; Key Examples", "Worked examples", "Core lesson.*", "Different actions.*", "Loud/soft.*", "Muffling.*", "Designing.*", "First Nations.*"]),
    findSectionAny(topicHtml, ["Worked application and transfer", "Guided learning activities", "Guided activities", "Classroom activities", "Support, core and extension", "Quick check for understanding"]),
    findSectionAny(topicHtml, ["Guided practice", "Independent practice", "Reasoning and problem-solving"]),
    findSectionAny(topicHtml, ["Curriculum elaborations explicitly taught", "Curriculum alignment", "Australian Curriculum elaborations", "Curriculum Coverage &amp; Elaborations", "Curriculum coverage and elaborations"]),
  ...
  const articles = [...sections.matchAll(/<article\b[^>]*>([\s\S]*?)<\/article>/gi)]
    .map((m) => stripTags(m[1]))
    .filter(Boolean);
  const modelText = [...sections.matchAll(/<div class="[^"]*(?:math-model-board|english-model-board|mini-visual|y4-subject-board|y4-vector-board|y4-subject-flow|y4-flow|y4-subject-cards|y4-card-row|y4-place|y4-compare|y4-number-grid|y4-array-wrap|y4-array|y4-fact-family|y4-number-line|y4-states|y5-board|y5-table|y5-cards|y5-compare|y5-sequence|y5-flow|y5-numberline|y5-fraction-set|y5-evidence|y6-board|y6-table|y6-cards|y6-compare|y6-sequence|y6-flow|y6-numberline|y6-fractions|y6-evidence|science-key|science-caution|curriculum-visual-cue|concept-card|example-card)[^"]*">([\s\S]*?)<\/div>/gi)]
    .map((m) => stripTags(m[1]))
    .filter(Boolean);
...
const buildExampleBoard = ({ code, subject, title, examples }) => {
  ...
  const cards = examples.map((item, index) => `<article class="example-card"><div class="example-label">Example ${index + 1}</div><div class="example-row"><div class="example-icon">${icon(subject, index)}</div><p>${esc(item)}</p></div></article>`).join("");
...
<header class="display-header"><div><p>Year 6 ${esc(titleCaseSubject(subject))}</p><h1 id="page-title">${esc(code)} - ${esc(title)}</h1><small>Teacher Display Page - plain HTML, landscape-friendly</small></div><p>Open one section at a time</p></header>
...
for (const file of files) {
  ...
  const code = firstMatch(topicHtml, /<p class="curriculum-eyebrow">([^<\s•]+)/) || firstMatch(topicHtml, /"curriculumCode":"([^"]+)"/) || firstMatch(fs.readFileSync(file, "utf8"), /<title>([A-Z0-9]+)/) || "Year6";
  ...
  const blocks = [
    section("Curriculum", "Start here", [findSectionAny(topicHtml, ["Learning goals: what you will learn", "What students learn", "What students learn in [^<]+", "Topic Guide"]), findSectionAny(topicHtml, ["Official curriculum reference", "Official curriculum references", "Curriculum Coverage & Resources", "Curriculum Coverage &amp; Resources"])]),
    section("Learning intention", "Say it simply", [findSectionAny(topicHtml, ["Learning goals: what you will learn", "What students learn", "What students learn in [^<]+", "Topic Guide"])]),
    buildExampleBoard({ code, subject, title: cleanTitle, examples }),
    section("Concept model", "Teach from the board", [findSectionAny(topicHtml, ["Key concept and central model", "Topic Guide", "Concept models and worked thinking", "Concept model and worked thinking", "Concept teaching", "Worked and modelled examples", "Concept model and worked application", "2\\. Subject-Specific Content &amp; Key Examples", "Worked examples", "Core lesson.*", "Different actions.*", "Loud/soft.*", "Muffling.*", "Designing.*", "First Nations.*"]), findSectionAny(topicHtml, ["Worked application and transfer"])]),
    section("Elaboration examples", "Copied content", [findSectionAny(topicHtml, ["Curriculum elaborations explicitly taught", "Curriculum Coverage & Resources", "Curriculum Coverage &amp; Resources", "Curriculum alignment", "Australian Curriculum elaborations", "Curriculum Coverage &amp; Elaborations", "Curriculum coverage and elaborations"])]),
    section("Important questions", "With answers", [findSectionAny(topicHtml, ["Important questions and answers", "Revision Notes and quick mastery check", "20 Must Questions with Worked Solutions", "Revision Notes", "Practice Thought &amp; Formative Assessment", "Quick check for understanding"])]),
    section("Assessment and review", "Ready for practice", [findSectionAny(topicHtml, ["Common misconceptions and corrections", "Common misconceptions and traps", "Common mistakes to watch for", "Common mistakes and rapid fixes", "Common misconceptions and quick fixes", "Revision Notes", "3\\. Common Student Misconceptions"]), findSectionAny(topicHtml, ["Assessment-style questions and review hints", "Resources and next steps", "Curriculum Coverage & Resources", "Curriculum Coverage &amp; Resources", "Practice and teaching resources", "Practice, test and teacher resources", "How to use this unit", "Practice Thought &amp; Formative Assessment"])]),
  ].filter(Boolean);
  if (!blocks.length) {
    skipped++;
    continue;
