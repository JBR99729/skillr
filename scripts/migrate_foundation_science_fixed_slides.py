...
def old_urls(code):
    lower=code.lower()
    return [
        f'/worksheets/foundation/science/teacher-slides/{lower}-teacher-slide.html',
        f'/worksheets/foundation/science/teacher-slides/live.html?code={code}',
        f'/worksheets/foundation/science/teacher-slides/live.html?code={lower}',
    ]

def patch_topic_links(topic,code):
    f=topic/'index.html'
    text=f.read_text(encoding='utf-8')
    for old in old_urls(code):
...
for code in CODES:
    ...
    if not pdf.exists():
        raise RuntimeError(f'Missing fixed PDF for {code}: {pdf}')
    ...
    if not generated:
        raise RuntimeError(f'No pages rendered for {code}')
...
if idx.exists():
    ...
    for code in CODES:
        slug=topic_dir_for(code).name
        replacement=f'/foundation/science/{slug}/teacher-slides/'
        for old in old_urls(code):
...
for f in TOPIC_ROOT.glob('*/index.html'):
    ...
    legacy_prefix='/worksheets/foundation/science/teacher-slides/'
    if legacy_prefix+'live.html' in text or (legacy_prefix+'ac9sf' in text and '-teacher-slide.html' in text):
