...
const units = JSON.parse(read('data/curriculum-units.json')).units
  .filter(unit => unit.yearNumber === 3 && unit.subjectSlug === 'science')
  .sort((a, b) => a.sourceOrder - b.sourceOrder);

if (units.length !== 12) failures.push(`Expected 12 Year 3 Science curriculum units; found ${units.length}`);
...
for (const unit of units) {
  ...
  if (!/\/assets\/year3-subject-visuals\.css\?v=\d+/i.test(html)) failures.push(`${code}: missing Year 3 subject visuals stylesheet`);

  const links = hrefs(html);
  for (const route of [`/quiz/year-3/science/${lowerCode}/worksheet/`, `/quiz/year-3/science/${lowerCode}/practice/`, `/quiz/year-3/science/${lowerCode}/test/`]) {
  ...
  if (links.some(href => /\/teacher-deck\/|\/worksheets\/year3\/science\/teacher-slides\/viewer\//i.test(href))) failures.push(`${code}: topic page must not point Teacher Slides at legacy/shared viewer route`);
...
const css = read('assets/year3-subject-visuals.css');
if (!css.includes(`.curriculum-page:has(.breadcrumb a[href="/year3/curriculum/science/"]) .curriculum-layout`)) failures.push('assets/year3-subject-visuals.css: missing Year 3 Science single-column layout selector');
if (!css.includes(`.curriculum-page:has(.breadcrumb a[href="/year3/curriculum/science/"]) .curriculum-sidebar`) || !css.includes('{display:none}')) failures.push('assets/year3-subject-visuals.css: Year 3 Science sidebar must be hidden for single-column reading');

if (failures.length) {
  console.error('Year 3 Science static topic-page validation failed:');
  for (const failure of failures) console.error(`- ${failure}`);
  process.exit(1);
}

console.log('PASS Year 3 Science static topic pages: 12/12 single-column, indexable, self-canonical, static and journey-safe.');
