...
const units = JSON.parse(read('data/curriculum-units.json')).units
  .filter(unit => unit.yearNumber === 5 && unit.subjectSlug === 'maths')
  .sort((a, b) => a.sourceOrder - b.sourceOrder);

if (units.length !== 24) failures.push(`Expected 24 Year 5 Maths curriculum units; found ${units.length}`);
...
for (const unit of units) {
  ...
  for (const route of [`/quiz/year-5/math/${lowerCode}/worksheet/`, `/quiz/year-5/math/${lowerCode}/practice/`, `/quiz/year-5/math/${lowerCode}/test/`]) {
  ...
  if (links.some(href => /\/worksheets\/year5\/maths\/teacher-slides\/viewer\//i.test(href))) failures.push(`${code}: topic page must not point Teacher Slides at shared viewer route`);
...
if (!css.includes(`.static-topic-page:has(.breadcrumb a[href="/year5/curriculum/maths/"]) .static-topic-grid`)) failures.push('assets/year5-curriculum.css: missing Year 5 Maths single-column layout selector');
if (!css.includes(`.static-topic-page:has(.breadcrumb a[href="/year5/curriculum/maths/"]) .static-topic-sidebar`) || !css.includes('{display:none}')) failures.push('assets/year5-curriculum.css: Year 5 Maths sidebar must be hidden for single-column reading');

if (failures.length) {
  console.error('Year 5 Maths static topic-page validation failed:');
  for (const failure of failures) console.error(`- ${failure}`);
  process.exit(1);
}

console.log('PASS Year 5 Maths static topic pages: 24/24 single-column, indexable, self-canonical, static and journey-safe.');
