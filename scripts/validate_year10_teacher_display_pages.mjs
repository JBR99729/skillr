...
for (const subject of subjects) {
  const base = path.join(root, 'year10', subject);
  if (!fs.existsSync(base)) {
    failures.push(`${subject}: missing Year 10 subject directory`);
    continue;
  }
...
if (!total) failures.push('no Year 10 Teacher Display pages were validated');

if (failures.length) {
  console.error('Year 10 Teacher Display validation FAILED');
  console.error(`Counts: English ${counts.english ?? 0}, Maths ${counts.maths ?? 0}, Science ${counts.science ?? 0}, total ${total}`);
  for (const failure of failures) console.error(`- ${failure}`);
  process.exit(1);
}

console.log(`Year 10 Teacher Display validation PASS (${total} pages: English ${counts.english}, Maths ${counts.maths}, Science ${counts.science})`);
