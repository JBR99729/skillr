...
for (const file of ["assets/year3-subject-data-base.js", "assets/year3-science-data.js"]) {
  vm.runInContext(fs.readFileSync(path.join(ROOT, file), "utf8"), context, { filename: file });
}
const units = context.window.SkillrYear3ScienceData;
...
for (const [code, unit] of Object.entries(units)) {
  ...
  for (const bankName of ["practice", "test"]) {
    ...
    html = html
      ...
      .replace(/<script src="[^"]*\/practice\/questions\.js(?:\?[^"']*)?"><\/script>/, `<script src="/quiz/year-3/science/${codeLower}/${bankName}/questions.js?v=20260813-year3-science-v1"></script>`)
      .replace(/questions\.js\?v=[^"]+/, `questions.js?v=20260813-year3-science-v1`);
