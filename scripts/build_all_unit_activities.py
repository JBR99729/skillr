...
def assessment_guidance(unit: dict, mode: str) -> list[str]:
    if mode == "test":
        return [
            ...
            "Aim for both accuracy and clear reasoning, then use Review answers to decide what to revisit.",
...
def attempt(unit: dict, mode: str, bank_url: str) -> str:
    ...
    config = {"storageKey": f"{unit['code']}{label}Best", "skillCode": unit["code"], "maxQuestions": 5, "shuffleQuestions": True, "shuffleAnswers": True, "questionCycle": False, "requireStudentName": mode == "test", "passingPercent": 75, "resultStorageKey": key, "resultUrl": "result/", "reviewUrl": "../review/", "retakeUrl": "../retake/"}
