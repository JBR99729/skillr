...
def card(p):
    ...
    buy = (f' <a class="button" href="{e(buy_url)}" target="_blank" rel="noopener noreferrer">Preview and buy — US$6.99</a>' if buy_url else '')
    bundle = (f' <a class="button" href="{e(p["bundleTptUrl"])}" target="_blank" rel="noopener noreferrer">Best value bundle — US${float(p["bundlePrice"]):.2f} (save 20%)</a>' if p.get('bundleTptUrl') else '')
