...
def build():
    ...
    for year in sorted({p['year'] for p in slides}):
        ...
        for subject in sorted({p['subject'] for p in year_products}):
            ...
            for p in ps:
                if p.get('resourceType') == 'teaching-sample':
                    action = f'<p class="price">Free sample</p><a class="button" href="{e(p["sampleUrl"])}">View sample slides</a> <a class="button" href="{e(p.get("samplePdf", p["sampleUrl"]))}">Download sample PDF</a>'+ (f' <a class="button" href="{e(p["tptUrl"])}" target="_blank" rel="noopener noreferrer">Get free sample on TPT</a>' if p.get('tptUrl') else '') + '<p>The full 60-slide teaching pack is being listed at US$6.99. The matching worksheet pack is US$2.99, and the planned TPT bundle is US$6.99 after a 30% discount.</p>'
                else:
