...
const failures = [];
const topicFiles = fs.readdirSync(path.join(root, 'year6/science'), {withFileTypes: true})
  .filter(entry => entry.isDirectory() && /^ac9s6/i.test(entry.name))
  .map(entry => `year6/science/${entry.name}/index.html`)
...
if (topicFiles.length !== 12) failures.push(`Expected 12 Year 6 Science topic pages; found ${topicFiles.length}`);
...
for (const topicFile of topicFiles) {
  ...
  for (const route of [`/quiz/year-6/science/${lowerCode}/practice/`, `/quiz/year-6/science/${lowerCode}/test/`]) {
  ...
  if (links.some(href => /\/worksheets\/year6\/science\/teacher-slides\/viewer\//i.test(href))) failures.push(`${code}: topic page must not point Teacher Slides at shared viewer route`);
...
if (!css.includes(`.static-topic-page:has(.breadcrumb a[href="/year6/curriculum/science/"]) .static-topic-grid`)) failures.push('assets/year6-curriculum.css: missing Year 6 Science single-column layout selector');
if (!css.includes(`.static-topic-page:has(.breadcrumb a[href="/year6/curriculum/science/"]) .static-topic-sidebar`) || !css.includes('{display:none}')) failures.push('assets/year6-curriculum.css: Year 6 Science sidebar must be hidden for single-column reading');

if (failures.length) {
  console.error('Year 6 Science static topic-page validation failed:');
  for (const failure of failures) console.error(`- ${failure}`);
  process.exit(1);
}

console.log('PASS Year 6 Science static topic pages: 12/12 single-column, indexable, self-canonical, static and journey-safe.');
