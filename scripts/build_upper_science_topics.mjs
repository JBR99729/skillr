...
function buildSpec(code,content,rawElaborations,slug){
  ...
  const assignment=profile.assignment?{
    heading:profile.assignment.heading,introduction:profile.assignment.introduction,
    items:[
      ...profile.assignment.extendedResponse.map((item,i)=>({id:`assignment-long-${i+1}`,type:"extendedResponse",number:i+1,...item})),
      ...profile.assignment.multipleChoice.map((item,i)=>({id:`assignment-mcq-${i+1}`,type:"multipleChoice",number:i+1,...item,expectedAnswer:`${String.fromCharCode(65+item.answerIndex)}. ${item.options[item.answerIndex]}`,acceptableEvidence:item.explanation,likelyError:"Selects a distractor that confuses the timing, chromosome behaviour or genetic consequence of the process.",remediation:item.explanation}))
    ]
  }:null;
  const assignmentSlides=assignment?.items.map(item=>({
    id:`slide-${item.id}`,title:item.type==="multipleChoice"?`Take-home multiple choice ${item.number}`:`Take-home extended response ${item.number}`,purpose:"Set the take-home task at the point where its concept is taught",
    display:{modelIds:[],studentPrompt:item.prompt,keyText:item.type==="multipleChoice"?["choose one","justify","submit"]:["explain","use evidence","submit"],assignmentItemId:item.id},
    teacherLayer:{teacherDoes:"Read the task once, clarify submission expectations and leave the scientific reasoning for students to complete independently.",teacherSaysOrAsks:item.prompt,studentDoes:item.type==="multipleChoice"?"Selects one option and records a one-sentence scientific justification.":"Writes a connected explanation using the lesson model and precise scientific vocabulary.",whatToLookFor:`Expected: ${item.expectedAnswer} Acceptable evidence: ${item.acceptableEvidence}`,ifIncorrect:`Likely error: ${item.likelyError} Remediation: ${item.remediation}`},
    checkpointIds:[],differentiationRefs:["support","core","extend"],elaborationIds:item.placement==="E2"?["E2"]:[]
  }))||[];
  const slidePlacement={central:"slide-intro",E2:"slide-e2",worked:"slide-worked"};
  const baseSlides=[introSlide,...elaborationSlides,workedSlide,finalSlide];
  const slides=baseSlides.flatMap(slide=>[slide,...assignmentSlides.filter((_,i)=>slidePlacement[assignment.items[i].placement]===slide.id)]);
  return {
    ...
    slides,masteryItems,...(assignment?{assignment}:{}),
...
function topicPage(u){
  ...
  const description=u.assignment?`${u.code} Year ${u.year} Science topic guide and teacher slides for meiosis, genetic variation, fertilisation and Mendelian inheritance, with take-home questions.`:`Teach ${u.code} with a complete Year ${u.year} Science topic guide, visual models, elaboration teaching, misconceptions, teacher slides and mastery checks.`;
