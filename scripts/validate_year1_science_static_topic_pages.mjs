...
const units = JSON.parse(read('data/curriculum-units.json')).units
  .filter(unit => unit.yearNumber === 1 && unit.subjectSlug === 'science')
  .sort((a, b) => a.sourceOrder - b.sourceOrder);

if (units.length !== 10) failures.push(`Expected 10 Year 1 Science curriculum units; found ${units.length}`);
...
for (const unit of units) {
  ...
  for (const route of [`/quiz/year-1/science/${lowerCode}/worksheet/`, `/quiz/year-1/science/${lowerCode}/practice/`, `/quiz/year-1/science/${lowerCode}/test/`]) {
...
if (!css.includes(`.curriculum-page:has(.breadcrumb a[href="/year1/curriculum/science/"]) .curriculum-layout`)) failures.push('assets/curriculum.css: missing Year 1 Science single-column layout selector');
if (!css.includes(`.curriculum-page:has(.breadcrumb a[href="/year1/curriculum/science/"]) .curriculum-sidebar`) || !css.includes('{display:none}')) failures.push('assets/curriculum.css: Year 1 Science sidebar must be hidden for single-column reading');

if (failures.length) {
  console.error('Year 1 Science static topic-page validation failed:');
  for (const failure of failures) console.error(`- ${failure}`);
  process.exit(1);
}

console.log('PASS Year 1 Science static topic pages: 10/10 single-column, indexable, self-canonical, static and journey-safe.');