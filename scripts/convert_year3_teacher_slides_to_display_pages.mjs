...
const baseDirs = ["year3/english", "year3/maths", "year3/science"];
...
const cleanTopicContent = (html) => html
  ...
  .replace(/\s*class="y3-subject-board"/g, ' class="display-model-board"')
  .replace(/\s*class="y3-vector-board"/g, ' class="display-model-board"')
  .replace(/\s*class="y3-subject-flow"/g, ' class="display-card-row"')
  .replace(/\s*class="y3-flow"/g, ' class="display-card-row"')
  .replace(/\s*class="y3-subject-cards"/g, ' class="display-card-row"')
  .replace(/\s*class="y3-card-row"/g, ' class="display-card-row"')
  .replace(/\s*class="y3-place"/g, ' class="display-card-row"')
  .replace(/\s*class="y3-compare"/g, ' class="display-model-board"')
  .replace(/\s*class="y3-number-grid"/g, ' class="display-card-row"')
  .replace(/\s*class="y3-array-wrap"/g, ' class="display-model-board"')
  .replace(/\s*class="y3-array"/g, ' class="display-card-row"')
  .replace(/\s*class="y3-fact-family"/g, ' class="display-model-board"')
  .replace(/\s*class="triangle"/g, ' class="display-card-row"')
  .replace(/\s*class="y3-number-line"/g, ' class="display-model-board"')
  .replace(/\s*class="rail"/g, ' class="display-card-row"')
  .replace(/\s*class="ends"/g, ' class="display-card-row"')
  .replace(/\s*class="y3-states"/g, ' class="display-card-row"')
...
const extractExamples = (topicHtml) => {
  ...
  const articles = [...sections.matchAll(/<article\b[^>]*>([\s\S]*?)<\/article>/gi)]
    .map((m) => stripTags(m[1]))
    .filter(Boolean);
  const modelText = [...sections.matchAll(/<div class="[^"]*(?:math-model-board|english-model-board|y3-subject-board|y3-vector-board|y3-subject-flow|y3-flow|y3-subject-cards|y3-card-row|y3-place|y3-compare|y3-number-grid|y3-array-wrap|y3-array|y3-fact-family|y3-number-line|y3-states|science-key|science-caution|curriculum-visual-cue|concept-card|example-card)[^"]*">([\s\S]*?)<\/div>/gi)]
    .map((m) => stripTags(m[1]))
    .filter(Boolean);
...
<header class="display-header"><div><p>Year 3 ${esc(titleCaseSubject(subject))}</p><h1 id="page-title">${esc(code)} - ${esc(title)}</h1><small>Teacher Display Page - plain HTML, landscape-friendly</small></div><p>Open one section at a time</p></header>
...
for (const file of files) {
  ...
  const code = firstMatch(topicHtml, /<p class="curriculum-eyebrow">([^<\s•]+)/) || firstMatch(topicHtml, /"curriculumCode":"([^"]+)"/) || firstMatch(fs.readFileSync(file, "utf8"), /<title>([A-Z0-9]+)/) || "Year3";
...
console.log(`Converted ${changed} Year 3 teacher slide files to display pages. Skipped ${skipped}.`);