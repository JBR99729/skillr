...
for(const [code,[src,skill]] of Object.entries(banks)){
 ...
 const pos=[0,0,0,0];[...P,...T].forEach(x=>pos[x.correct]++);if(Math.max(...pos)-Math.min(...pos)>2)throw Error(`${code} answer-position imbalance ${pos}`);
