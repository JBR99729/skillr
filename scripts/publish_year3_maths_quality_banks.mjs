...
for (const [code, unit] of Object.entries(units)) {
  ...
  const bankFile = `assets/assessment-banks/year3/math/${codeLower}.json`;
  execFileSync(process.execPath, ["scripts/publish_production_question_bank.mjs", bankFile], { cwd: ROOT, stdio: "inherit" });
  const route = path.join(ROOT, "quiz", "year-3", "math", codeLower);
  ...
  for (const bankName of ["practice", "test"]) {
    ...
    const description = isTest
      ? `Take an 8-question Year 3 ${unit.title} test drawn from a separate 16-question bank.`
    ...
    html = html
      ...
      .replace(/<script src="[^"]*\/practice\/questions\.js(?:\?[^"']*)?"><\/script>/, `<script src="/quiz/year-3/math/${codeLower}/${bankName}/questions.js?v=20260813-year3-maths-v1"></script>`)
      .replace(/questions\.js\?v=[^"]+/, `questions.js?v=20260813-year3-maths-v1`);
...
console.log(JSON.stringify({ codes: Object.keys(units).length, practiceBank: 24, testBank: 16, practiceAttempt: 8, testAttempt: 8, status: "PUBLISHED" }, null, 2));
