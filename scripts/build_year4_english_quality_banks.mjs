...
const BANK_ROOT = path.join(ROOT, "assets", "assessment-banks", "year4", "english");
const VISUAL_ROOT = path.join(ROOT, "assets", "assessment-visuals", "year4", "english");
const raw = {}; const rawOrder = [];
const context = { window: { SkillrYear4SubjectRegister(subject, specs, order) { if (subject === "english") { Object.assign(raw, specs); rawOrder.push(...order); } } } };
vm.createContext(context);
for (const file of ["assets/year4-english-data-la1.js","assets/year4-english-data-la2.js","assets/year4-english-data-le.js","assets/year4-english-data-ly1.js","assets/year4-english-data-ly2.js"]) vm.runInContext(fs.readFileSync(path.join(ROOT,file),"utf8"),context,{filename:file});
const units = raw;
const codes = [...new Set(rawOrder)];
if (codes.length !== 28) throw new Error(`Expected 28 English codes, found ${codes.length}`);

const clean = (value) => String(value ?? "").replace(/\{\{blank\}\}|_+/gi, "the missing word").replace(/\/+/g, " or ").replace(/\s+/g, " ").trim();
...
function dataContexts(code, unit) {
  ...
  const choices = [unit.questions?.choice1, unit.questions?.choice2].filter(Boolean);
  const make = (bank) => {
    const rows = [];
    for (const [index, choice] of choices.entries()) {
      const [question, answers] = choice;
      rows.push(source(`${bank === "test" ? "New text" : "Worked text"} ${index + 1}`, `${bank === "test" ? "In a new assessment text, " : "Read the classroom example. "}${clean(question)}`, answers[0], answers.slice(1, 3), `The evidence and language purpose support “${answers[0]}”.`, "Check the whole sentence or text feature, then test meaning and grammar.", "Read, compare and justify"));
    }
    mistakes.forEach(([claim, correction], index) => rows.push(source(`Misconception ${index + 1}`, `${bank === "test" ? "An editor reviewing a new text" : "A student in a workshop"} claims, “${clean(claim)}”. Which response gives the most accurate correction?`, correction, [claim, mistakes[(index + 1) % mistakes.length][0]], correction, "Identify exactly what the claim overlooks, then choose the correction that restores meaning.", "Claim, evidence, correction")));
    activities.forEach(([title, description], index) => rows.push(source(title, `${bank === "test" ? "A different class adapts" : "The class prepares"} “${clean(title)}”. Which plan best demonstrates the English skill?`, description, [`Copy the title, then choose an answer without analysing the language or text.`, `Change the purpose and audience halfway through, then report the first impression only.`], description, "Choose the plan that analyses language evidence and produces a reviewable response.", "Plan, apply, review")));
    terms.forEach(([term, definition], index) => { const other=terms.filter((_,i)=>i!==index).map(x=>x[0]); rows.push(source(`Vocabulary ${index + 1}`, `${bank === "test" ? "A museum guide for a new text" : "A class annotation"} about ${unit.title} describes “${clean(definition)}”. Which term is most precise?`, term, other.slice(0,2), `${term} is the precise term for this feature.`, "Match the definition to the exact language or literacy concept.", "Definition and example")); });
    return rows.slice(0, bank === "practice" ? 12 : 8);
  };
  return { practice: make("practice"), test: make("test") };
}
...
function editingChoiceSpeech(value) {
  ...
  parts.push(/^[“\"]/.test(text) ? "An opening quotation mark comes before the first word." : "There is no opening quotation mark before the first word.");
  if (/,[”\"]/.test(text)) parts.push("The comma comes before the closing quotation mark.");
  else if (/[”\"],/.test(text)) parts.push("The closing quotation mark comes before the comma.");
  else parts.push("There is no comma beside a closing quotation mark.");
...
function spellingChoiceSpeech(value) {
  const text = clean(value);
  const letters = [...text.replace(/[^A-Za-z]/g, "")].map(letter => letter.toUpperCase()).join(", ");
  return `The written choice is spelled ${letters}. The word or phrase reads: ${text}`;
}
...
function makeItem(code, unit, bank, bankIndex, itemSource, variant, sourceIndex) {
  ...
  const correct = isAnswer ? itemSource.correct : `${itemSource.hint} ${itemSource.summary}`;
  ...
  return {
    item: {
      ...
      visual: { type: "svg", asset_path: `/assets/assessment-visuals/year4/english/${code.toLowerCase()}.svg#${symbolId}`, alt_text: visualAlt },
      answers,
      ...(/(?:LY09|LY10|LY11)$/.test(code) ? { audio_answers: answers.map((answer) => spellingChoiceSpeech(answer.text)) } : /(?:LA07|LA08|LA09|LA10|LA11|LA12)$/.test(code) ? { audio_answers: answers.map((answer) => editingChoiceSpeech(answer.text)) } : {}),
...
fs.writeFileSync(path.join(BANK_ROOT, "curriculum-map.json"), `${JSON.stringify(Object.fromEntries(codes.map(code => [code, {
  title: units[code].title,
  curriculum: units[code].desc,
  misconceptions: units[code].mistakes.map(([misconception, correction]) => ({ misconception, correction })),
  practiceContexts: ["worked classroom texts", "student editing workshop", "guided language investigation"],
  testContexts: ["new assessment texts", "independent editor review", "museum and publication tasks"],
  visualFocus: ["read", "notice", "apply or correct"]
}])), null, 2)}\n`);
