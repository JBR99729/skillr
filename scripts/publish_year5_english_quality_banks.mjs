...
for (const file of ["assets/year5-curriculum-base.js", "assets/year5-english-data-la.js", "assets/year5-english-data-le.js", "assets/year5-english-data-ly1.js", "assets/year5-english-data-ly2.js"]) vm.runInContext(fs.readFileSync(path.join(ROOT, file), "utf8"), sandbox, { filename: file });
const units = sandbox.window.SkillrYear5EnglishData;
const order = sandbox.window.SkillrYear5EnglishOrder;
if (!units || order?.length !== 24) throw new Error("Expected all 24 Year 5 English units");
...
for (const code of order) {
  ...
  execFileSync(process.execPath, ["scripts/publish_production_question_bank.mjs", `assets/assessment-banks/year5/english/${lower}.json`], { cwd: ROOT, stdio: "inherit" });
  const route = path.join(ROOT, "quiz", "year-5", "english", lower);
  const misconception = unit.mistakes?.[0] ?? ["A feature is named without explaining its effect", "Use evidence from the text and connect it to audience and purpose."];
  const notes = [unit.learn, `Use this routine: ${unit.routine}.`, `Watch for ${misconception[0].toLowerCase()}: ${misconception[1]}`];
  for (const bank of ["practice", "test"]) {
    ...
    html = html
      ...
      .replace(/<script src="[^"]*\/practice\/questions\.js(?:\?[^"]*)?"><\/script>/, `<script src="/quiz/year-5/english/${lower}/${bank}/questions.js?v=20260813-production-v1"></script>`)
