...
const units = JSON.parse(read('data/curriculum-units.json')).units
  .filter(unit => unit.yearNumber === 2 && unit.subjectSlug === 'science')
  .sort((a, b) => a.sourceOrder - b.sourceOrder);

if (units.length !== 10) failures.push(`Expected 10 Year 2 Science curriculum units; found ${units.length}`);
...
for (const unit of units) {
  ...
  const legacyStaticSectionPage = code === 'AC9S2U01';
  ...
  if (!legacyStaticSectionPage && !/<details\b/i.test(html)) failures.push(`${code}: migrated topic pages must keep native details/summary sections`);

  const links = hrefs(html);
  for (const route of [`/quiz/year-2/science/${lowerCode}/worksheet/`, `/quiz/year-2/science/${lowerCode}/practice/`, `/quiz/year-2/science/${lowerCode}/test/`]) {
...
if (!css.includes(`.curriculum-page:has(.breadcrumb a[href="/year2/curriculum/science/"]) .curriculum-layout`)) failures.push('assets/curriculum.css: missing Year 2 Science single-column layout selector');
if (!css.includes(`.curriculum-page:has(.breadcrumb a[href="/year2/curriculum/science/"]) .curriculum-sidebar`) || !css.includes('{display:none}')) failures.push('assets/curriculum.css: Year 2 Science sidebar must be hidden for single-column reading');

if (failures.length) {
  console.error('Year 2 Science static topic-page validation failed:');
  for (const failure of failures) console.error(`- ${failure}`);
  process.exit(1);
}

console.log('PASS Year 2 Science static topic pages: 10/10 single-column, indexable, self-canonical, static and journey-safe.');
