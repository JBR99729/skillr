...
const version='20260908-year4-second-five';
...
assert.equal(year4Codes.filter(c=>ledger.has(c)).length,10,'The first ten codes must be approved in this release');
for(const code of ['AC9M4N01','AC9M4N02','AC9M4N03','AC9M4N04','AC9M4N05'])assert(ledger.has(code),`${code}: preserve first-five review`);
...
const output={status:'PASS',version,reviewed:10,totalCodes:23,badgeActivated:false,results};
fs.mkdirSync(path.join(root,'reports/year4-maths-ixl-review'),{recursive:true});
fs.writeFileSync(path.join(root,'reports/year4-maths-ixl-review/SECOND-FIVE-VALIDATION.json'),JSON.stringify(output,null,2)+'\n');
