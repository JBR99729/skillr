...
const baseDirs = ["year5/english", "year5/maths", "year5/science"];
...
const extractExamples = (topicHtml) => {
  let sections = [
    findSectionAny(topicHtml, ["Concept models and worked thinking", "Concept model and worked thinking", "Worked and modelled examples", "Concept teaching", "Concept model and worked application", "2\\. Subject-Specific Content &amp; Key Examples", "Worked examples", "Core lesson.*", "Different actions.*", "Loud/soft.*", "Muffling.*", "Designing.*", "First Nations.*"]),
  ...
  const articles = [...sections.matchAll(/<article\b[^>]*>([\s\S]*?)<\/article>/gi)]
    .map((m) => stripTags(m[1]))
    .filter(Boolean);
  const modelText = [...sections.matchAll(/<div class="[^"]*(?:math-model-board|english-model-board|y4-subject-board|y4-vector-board|y4-subject-flow|y4-flow|y4-subject-cards|y4-card-row|y4-place|y4-compare|y4-number-grid|y4-array-wrap|y4-array|y4-fact-family|y4-number-line|y4-states|y5-board|y5-table|y5-cards|y5-compare|y5-sequence|y5-flow|y5-numberline|y5-fraction-set|y5-evidence|science-key|science-caution|curriculum-visual-cue|concept-card|example-card)[^"]*">([\s\S]*?)<\/div>/gi)]
    .map((m) => stripTags(m[1]))
    .filter(Boolean);
...
<header class="display-header"><div><p>Year 5 ${esc(titleCaseSubject(subject))}</p><h1 id="page-title">${esc(code)} - ${esc(title)}</h1><small>Teacher Display Page - plain HTML, landscape-friendly</small></div><p>Open one section at a time</p></header>
...
for (const file of files) {
  ...
  const code = firstMatch(topicHtml, /<p class="curriculum-eyebrow">([^<\s•]+)/) || firstMatch(topicHtml, /"curriculumCode":"([^"]+)"/) || firstMatch(fs.readFileSync(file, "utf8"), /<title>([A-Z0-9]+)/) || "Year5";
  ...
  const blocks = [
    ...
    section("Concept model", "Teach from the board", [findSectionAny(topicHtml, ["Concept models and worked thinking", "Concept model and worked thinking", "Concept teaching", "Worked and modelled examples", "Concept model and worked application", "2\\. Subject-Specific Content &amp; Key Examples", "Worked examples", "Core lesson.*", "Different actions.*", "Loud/soft.*", "Muffling.*", "Designing.*", "First Nations.*"])]),
...
console.log(`Converted ${changed} Year 5 teacher slide files to display pages. Skipped ${skipped}.`);
