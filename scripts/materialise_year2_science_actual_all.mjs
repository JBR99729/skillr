...
for(const code of codes){const d=direct32(code);if(d.length!==32)throw Error(`${code} direct count ${d.length}`);const x=extras[code];if(!x||x.practice.length!==8||x.test.length!==16)throw Error(`${code} extension counts`);const rawP=[...d,...x.practice],rawT=x.test;const seen=new Set();for(const r of [...rawP,...rawT]){const q=norm(cleanRow(r).q);if(seen.has(q))throw Error(`${code} duplicate ${q}`);seen.add(q)}
