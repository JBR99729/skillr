...
const units = JSON.parse(read('data/curriculum-units.json')).units
  .filter(unit => unit.yearNumber === 5 && unit.subjectSlug === 'science')
  .sort((a, b) => a.sourceOrder - b.sourceOrder);

if (units.length !== 12) failures.push(`Expected 12 Year 5 Science curriculum units; found ${units.length}`);
...
for (const unit of units) {
  ...
  if (/id=["'](?:topicRoot|year5Topic)["'][^>]*>\s*(?:<p[^>]*>)?\s*Loading/i.test(html) || /year5-curriculum-render\.js|topic-modules-render|lesson-render/i.test(html)) failures.push(`${code}: topic page must not depend on runtime curriculum rendering`);
  ...
  for (const route of [`/quiz/year-5/science/${lowerCode}/worksheet/`, `/quiz/year-5/science/${lowerCode}/practice/`, `/quiz/year-5/science/${lowerCode}/test/`]) {
  ...
  if (links.some(href => /\/worksheets\/year5\/science\/teacher-slides\/viewer\//i.test(href))) failures.push(`${code}: topic page must not point Teacher Slides at shared viewer route`);
...
const css = read('assets/year5-curriculum.css');
if (!css.includes(`.static-topic-page:has(.breadcrumb a[href="/year5/curriculum/science/"]) .static-topic-grid`)) failures.push('assets/year5-curriculum.css: missing Year 5 Science single-column layout selector');
if (!css.includes(`.static-topic-page:has(.breadcrumb a[href="/year5/curriculum/science/"]) .static-topic-sidebar`) || !css.includes('{display:none}')) failures.push('assets/year5-curriculum.css: Year 5 Science sidebar must be hidden for single-column reading');

if (failures.length) {
  console.error('Year 5 Science static topic-page validation failed:');
  for (const failure of failures) console.error(`- ${failure}`);
  process.exit(1);
}

console.log('PASS Year 5 Science static topic pages: 12/12 single-column, indexable, self-canonical, static and journey-safe.');
