...
const baseDirs = ["year7/english", "year7/maths", "year7/science"];
...
const findSection = (html, label) => {
  const detailsMarker = new RegExp(`<details[^>]*>\\s*<summary>\\s*(?:<strong>)?${label}(?:<\\/strong>)?\\s*<\\/summary>\\s*<div class="[^"]*\\b(?:curriculum-detail-body|static-topic-panel)\\b[^"]*">`, "i");
  ...
  const genericDetailsMarker = new RegExp(`<details[^>]*>\\s*<summary>\\s*(?:<strong>)?${label}(?:<\\/strong>)?\\s*<\\/summary>`, "i");
...
<header class="display-header"><div><p>Year 7 ${esc(titleCaseSubject(subject))}</p><h1 id="page-title">${esc(code)} - ${esc(title)}</h1><small>Teacher Display Page - plain HTML, landscape-friendly</small></div><p>Open one section at a time</p></header>
...
for (const file of files) {
  ...
  const code = firstMatch(topicHtml, /<p class="curriculum-eyebrow">([^<\s•]+)/) || firstMatch(topicHtml, /"curriculumCode":"([^"]+)"/) || firstMatch(fs.readFileSync(file, "utf8"), /<title>([A-Z0-9]+)/) || "Year7";
...
console.log(`Converted ${changed} Year 7 teacher slide files to display pages. Skipped ${skipped}.`);
