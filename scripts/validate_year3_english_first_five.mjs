...
const baseline=process.argv[2]||'4a808a4acae1aca0e2cc11ae9f48872b9b187a98';
...
for(let n=1;n<=5;n++){
 const code=`AC9E3LA0${n}`,lower=code.toLowerCase(),sourcePath=`assets/assessment-banks/year3/english/${lower}.json`;
 ...
 for(const q of source){
  ...
  assert.equal(q.visual.type,'none');assert(!q.visual.asset_path&&!q.visual.alt_text);
 ...
 for(const bank of ['practice','test']){
  ...
  assert(html.includes('20260907-english-first-five'));
...
fs.writeFileSync(path.join(root,'reports/year3-english-ixl-review/FIRST-FIVE-VALIDATION.json'),JSON.stringify(output,null,2)+'\n');
