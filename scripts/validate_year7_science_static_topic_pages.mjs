...
for (const unit of units) {
  ...
  if (!exists(teacherViewer)) failures.push(`${unit.code}: missing clean teacher-slides viewer`);
    ...
    if (!(/<img\b[^>]+slide-/i.test(viewer) || /\bdata-slide\b/i.test(viewer)) || !/(?:Previous|data-prev)/i.test(viewer) || !/(?:Next|data-next)/i.test(viewer)) failures.push(`${unit.code}: teacher-slides viewer must expose fixed slide pages and navigation`);
