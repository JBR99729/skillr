...
for (const code of codes) {
  ...
  const bankFile = `assets/assessment-banks/year2/english/${lower}.json`;
  ...
  for (const bank of ["practice", "test"]) {
    ...
    const description = bank === "practice"
      ? `Practise Year 2 ${unit.title} with 8 rotating questions from a 24-question bank.`
    ...
    html = html
      ...
      .replace(/<script>window\.skillrYear2EnglishBankConfig=[\s\S]*?<\/script>/g, "")
      .replace(/<script src="\/assets\/year2-english-authored-banks[^>]*><\/script>/g, "")
      .replace(/<script src="\/assets\/year2-english-bank-loader[^>]*><\/script>/g, "")
      .replace(/<script src="[^\"]*\/(?:practice\/)?questions\.js(?:\?[^\"]*)?"><\/script>/g, "")
      .replace(/<script src="\/quiz\/assets\/production-question-ui\.js[^>]*><\/script>/g, "")
      .replace(/<script src="\/assets\/year2-english-assessment-ui\.js[^>]*><\/script>/g, "")
      .replace(/<script src="\/quiz\/assets\/script\.js[^>]*><\/script>/, `<script src="/quiz/year-2/english/${lower}/${bank}/questions.js?v=20260813-production-v1"></script><script src="/assets/year2-english-assessment-ui.js?v=1"></script>$&`);
  ...
  for (const file of filesUnder(route)) {
    ...
    if (relative === "worksheet/index.html") {
      html = html
        ...
        .replace(/<meta name="description" content="[^"]*">/, `<meta name="description" content="Download a separate Year 2 ${unit.title} worksheet aligned to ${code}.">`);
    }
    fs.writeFileSync(file, html);
  }
}
