...
for (const file of ["assets/year5-curriculum-base.js", "assets/year5-science-data.js"]) {
  vm.runInContext(fs.readFileSync(path.join(ROOT, file), "utf8"), sandbox, { filename: file });
}
const units = sandbox.window.SkillrYear5ScienceData;
const order = sandbox.window.SkillrYear5ScienceOrder;
if (!units || order?.length !== 12) throw new Error("Expected all 12 Year 5 Science units");
...
for (const code of order) {
  ...
  execFileSync(process.execPath, ["scripts/publish_production_question_bank.mjs", `assets/assessment-banks/year5/science/${lower}.json`], { cwd: ROOT, stdio: "inherit" });
  const route = path.join(ROOT, "quiz", "year-5", "science", lower);
  const misconception = unit.mistakes?.[0] ?? ["A first idea may not fit the evidence", "Compare the claim with observations before deciding."];
  const notes = [unit.learn, `Use evidence to ${unit.quick?.[0]?.toLowerCase() ?? "explain the pattern"}`, `Watch for ${misconception[0].toLowerCase()}: ${misconception[1]}`];
  ...
  for (const bank of ["practice", "test"]) {
    ...
    html = html
      ...
      .replace(/<script src="[^"]*\/practice\/questions\.js(?:\?[^"]*)?"><\/script>/, `<script src="/quiz/year-5/science/${lower}/${bank}/questions.js?v=20260813-production-v1"></script>`)
