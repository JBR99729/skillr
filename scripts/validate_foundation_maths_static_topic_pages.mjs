...
if (units.length !== 12) failures.push(`Expected 12 Foundation Maths curriculum units; found ${units.length}`);
...
for (const unit of units) {
  ...
  if (!/<body\b[^>]*class=["'][^"']*\bfoundation-maths-topic\b/i.test(html)) failures.push(`${code}: topic page must keep Foundation Maths scope class`);
  ...
  for (const route of [`/quiz/grade-k/math/${lowerCode}/worksheet/`, `/quiz/grade-k/math/${lowerCode}/practice/`, `/quiz/grade-k/math/${lowerCode}/test/`]) {
...
if (!css.includes(`.foundation-maths-topic .curriculum-layout`)) failures.push('assets/curriculum.css: missing Foundation Maths single-column layout selector');
if (!css.includes(`.foundation-maths-topic .curriculum-sidebar`) || !css.includes('{display:none}')) failures.push('assets/curriculum.css: Foundation Maths sidebar must be hidden for single-column reading');

if (failures.length) {
  console.error('Foundation Maths static topic-page validation failed:');
  for (const failure of failures) console.error(`- ${failure}`);
  process.exit(1);
}

console.log('PASS Foundation Maths static topic pages: 12/12 single-column, indexable, self-canonical, static and journey-safe.');
