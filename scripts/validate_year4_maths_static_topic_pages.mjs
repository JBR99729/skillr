...
const units = JSON.parse(read('data/curriculum-units.json')).units
  .filter(unit => unit.yearNumber === 4 && unit.subjectSlug === 'maths')
  .sort((a, b) => a.sourceOrder - b.sourceOrder);

if (units.length !== 23) failures.push(`Expected 23 Year 4 Maths curriculum units; found ${units.length}`);
...
for (const unit of units) {
  ...
  if (/id=["'](?:topicRoot|year4Topic)["'][^>]*>\s*(?:<p[^>]*>)?\s*Loading/i.test(html) || /year4-maths-render\.js|topic-modules-render|lesson-render/i.test(html)) failures.push(`${code}: topic page must not depend on runtime curriculum rendering`);
  ...
  for (const route of [`/quiz/year-4/math/${lowerCode}/worksheet/`, `/quiz/year-4/math/${lowerCode}/practice/`, `/quiz/year-4/math/${lowerCode}/test/`]) {
  ...
  if (links.some(href => /\/worksheets\/year4\/maths\/teacher-slides\/viewer\//i.test(href))) failures.push(`${code}: topic page must not point Teacher Slides at shared viewer route`);
...
const css = read('assets/year4-maths-visuals.css');
if (!css.includes(`.curriculum-page:has(.breadcrumb a[href="/year4/curriculum/maths/"]) .curriculum-layout`)) failures.push('assets/year4-maths-visuals.css: missing Year 4 Maths single-column layout selector');
if (!css.includes(`.curriculum-page:has(.breadcrumb a[href="/year4/curriculum/maths/"]) .curriculum-sidebar`) || !css.includes('{display:none}')) failures.push('assets/year4-maths-visuals.css: Year 4 Maths sidebar must be hidden for single-column reading');

if (failures.length) {
  console.error('Year 4 Maths static topic-page validation failed:');
  for (const failure of failures) console.error(`- ${failure}`);
  process.exit(1);
}

console.log('PASS Year 4 Maths static topic pages: 23/23 single-column, indexable, self-canonical, static and journey-safe.');