  ...
  for(const bank of ['practice','test']){
    ...
    for(const q of selected){
      ...
      if(q.type==='self-check'){
        ...
        assert(q.modelAnswer && q.acceptanceNote.startsWith('Adult check:'));
        assert(html.includes('"requireAdultReviewSupport":true'));
        assert(html.includes('/quiz/assets/year1-maths-support.js'));
      }else{
