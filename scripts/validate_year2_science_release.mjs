...
const revised24Codes=new Set(['ac9s2u02','ac9s2u03','ac9s2h01']);
...
for(const code of codes){
  ...
  for(const bank of ['practice','test']){
    ...
    const expected=bank==='practice'?(revised24Codes.has(code)?24:48):16;
    ...
    const version=revised24Codes.has(code)?'20260906-y2-science-24-16':'20260905-y2-science-reviewed';
    assert(html.includes(`/${code}/${bank}/questions.js?v=${version}`));
    ...
    for(const match of html.matchAll(/\b(\d+)-question bank/g)) assert.equal(Number(match[1]),expected,'Stale count '+code+' '+bank);
