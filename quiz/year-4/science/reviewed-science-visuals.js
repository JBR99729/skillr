...
document.addEventListener("DOMContentLoaded", () => {
  const code = location.pathname.match(/\/science\/(ac9s4(?:u0[1-4]|h0[12]|i0[1-6]))\//)?.[1];
  ...
  document.querySelectorAll("#answerReviewList .review-item").forEach((card, index) => {
    ...
    const localModel = new RegExp(`^/assets/(?:assessment-visuals/year4/science/${code}|science-models/year4/${code})(?:[-/][a-zA-Z0-9_-]+)*\\.svg#[a-zA-Z0-9_-]+$`);
    if (visual?.type !== "svg" || !localModel.test(visual.asset_path || "")) return;
    const heading = card.querySelector("h2");
    if (!heading || card.querySelector(".reviewed-science-model")) return;
    const svg = document.createElementNS(namespace, "svg");
    svg.classList.add("reviewed-science-model");
