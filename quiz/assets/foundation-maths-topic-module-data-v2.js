...
// Scoped text review: position and data.
(() => {
 const edits = {"AC9MFSP02": [{"prompt": null, "answer": "inside. The ball is in the box.", "hint": "Use the box as the reference object."}, {"prompt": "A toy is directly below a chair seat, between the chair legs.", "answer": "under. The toy is directly below the chair seat.", "hint": "Name both the position and the chair."}, {"prompt": null, "answer": "inside → in the box; under → below the table; beside → next to the chair.", "hint": "Match each word to its relationship with the named object."}, {"prompt": null, "answer": "Put it beside the desk. This names a position and a reference object.", "hint": "Look for information that helps someone find the intended place."}, {"prompt": null, "answer": "For example, “Put the toy under the chair.”", "hint": "Accept an instruction that clearly names the toy, position and chair."}, {"prompt": "The pencil rests on the upper cover of a closed book. Complete the location sentence.", "answer": "on top of; “on” is also acceptable. The pencil rests on the book’s upper cover.", "hint": "Use the location described in the question."}, {"prompt": null, "answer": "go forward → move ahead; turn left → change direction left; stop beside the table → finish next to the table.", "hint": "Use the moving person’s viewpoint for left and forward."}, {"prompt": "There are two boxes: one beside the chair and one beside the door. The ball is inside the box beside the chair. Explain how both clues help you find it.", "answer": "Inside tells where the ball is in its box; beside the chair identifies which of the two boxes.", "hint": "Use both clues to select the location."}, {"prompt": null, "answer": "“Over there” gives no named reference. “Inside the box beside the chair” identifies the container and where to find it.", "hint": "Explain how the listener can locate the object without guessing."}], "AC9MFST01": [{"prompt": "Each counter represents one response. Which category has more responses?", "answer": "Cats. Four counters represent cats and two represent dogs.", "hint": "Use one counter per response and count each category."}, {"prompt": null, "answer": "1. Pair three apple responses with the three banana responses; one apple response remains.", "hint": "Match the groups one to one."}, {"prompt": null, "answer": "collect → gather responses; sort → put responses into categories; compare → look for more, less or same.", "hint": "Connect each word to the step it describes."}, {"prompt": null, "answer": "So comparison is fair and easy. Equal-sized counters with equal spacing and a shared start make the counts easier to compare.", "hint": "Spacing can change how long a row looks without changing its count."}, {"prompt": null, "answer": "For example, toys with wheels and toys with no wheels. Every toy goes in one group.", "hint": "Choose a clear rule that places every toy exactly once."}, {"prompt": null, "answer": "same. Both groups have five votes.", "hint": "Equal counts mean neither category has more."}, {"prompt": null, "answer": "6 vs 3 → first has more; 2 vs 5 → first has fewer; 4 vs 4 → same.", "hint": "Compare the first count with the second in each pair."}, {"prompt": "Each counter represents one vote. One row has 4 close together; another has 3 spread far apart. Does the wider row have more votes? Explain.", "answer": "No. Four counters represent more responses than three, even when the three are spread farther apart.", "hint": "Count the counters or line them up with equal spacing to check."}, {"prompt": null, "answer": "The toy is counted twice, so the display can overstate the total. Apply the rule and remove its extra entry so the toy appears once.", "hint": "Check that each object has one matching representation."}]};
 for (const [code, rows] of Object.entries(edits)) {
  const unit = window.SkillrFoundationWorksheetData?.[code];
  if (!unit || unit.questions.length !== rows.length) continue;
  unit.questions.forEach((q,i) => {
