...
(() => {
  ...
  function renderQuestion(question, index) {
    ...
    return `<article class="worksheet-question${question.enrichment ? " enrichment" : ""}"><div class="question-line"><span class="question-number-text">${index + 1}.</span>${question.enrichment ? '<span class="enrichment-label">Enrichment</span>' : ""}<p class="question-prompt">${esc(question.question)}</p></div>${question.visual ? `<div class="question-visual">${esc(question.visual)}</div>` : ""}${response}</article>`;
  }
  ...
  function drawOuterBorder(doc, pageW, pageH) { doc.setDrawColor(36,87,214); doc.setLineWidth(.55); doc.rect(5,5,pageW-10,pageH-10); }
  ...
  function drawQuestion(doc, question, number, x, y, width, height) {
    ...
    } else if (question.type === "fill-blank") { doc.setFont("helvetica","bold"); doc.setFontSize(10); doc.text(String(question.template||"").replaceAll("{{blank}}","__________"),x+8,cursor); }
    else if (question.type === "match") { const left=question.matchLeft||[], right=question.matchRight||[], count=Math.max(left.length,right.length); doc.setFontSize(8.8); for(let i=0;i<count;i++){doc.text(left[i]!==undefined?`${String.fromCharCode(65+i)}. ${left[i]}`:"",x+8,cursor+i*4.1); doc.text(right[i]!==undefined?`${i+1}. ${right[i]}`:"",x+width*.57,cursor+i*4.1);} const ly=cursor+count*4.1+1; doc.setDrawColor(...muted); doc.line(x+8,ly,x+width-4,ly); }
    else { const count=question.enrichment?4:2; doc.setDrawColor(...muted); for(let i=0;i<count;i++){const ly=cursor+i*5.2; if(ly<y+height-1) doc.line(x+8,ly,x+width-4,ly);} }
  ...
  async function previewPdf() {
    ...
    try {
      ...
      pages.forEach((items,pi)=>{if(pi>0)doc.addPage("letter","portrait"); drawWatermark(doc,pageW,pageH); drawOuterBorder(doc,pageW,pageH); let y=drawHeader(doc,pageW,pi+1,count); items.forEach((item)=>{drawQuestion(doc,item.question,item.number,m,y,width,item.height);y+=item.height+2.4;}); drawFooter(doc,pageW,pageH,pi+1,count);});
