...
(() => {
  ...
  const READING_CONTEXTS=["Before choosing, a reader returns to the passage","A student checks the exact words in the text","A reader looks for the strongest clue","A class compares two interpretations","A student traces the order of events","A reader checks who is speaking or acting","A class identifies the important detail","A student separates a fact from an inference","A reader checks the setting","A class discusses the main idea","A student looks for evidence in two sentences","A reader checks why an event happened","A class compares the beginning and ending","A student explains a character's choice","A reader notices a repeated idea","A class checks the meaning of a word in context","A student summarises the passage","A reader checks what changed","A class evaluates a possible conclusion","A student identifies the problem and response","A reader checks the author's wording","A class distinguishes a detail from the main message","A student tests an inference against the text","A reader rereads the relevant sentence","A class completes a final evidence check"];
  const WORD_CONTEXTS=["A student chooses a precise word","A class edits a sentence","A writer checks meaning in context","A student compares three word choices","A class improves a description","A writer replaces a vague word","A student checks spelling and meaning","A class sorts words by meaning","A writer chooses the clearest expression","A student uses a context clue","A class checks a word family","A writer avoids repetition","A student compares formal and informal wording","A class completes a sentence","A writer checks a word's effect","A student explains a word to a partner","A class chooses an accurate synonym","A writer checks an antonym","A student corrects a word choice","A class reads the whole sentence aloud","A writer checks tone","A student identifies the root or affix","A class compares possible meanings","A writer makes the sentence more precise","A student completes a final meaning check"];
  const LANGUAGE_CONTEXTS=["A student edits a draft","A class checks a sentence","A writer rereads for clarity","A student fixes punctuation","A class compares two revisions","A writer checks the verb tense","A student joins ideas clearly","A class removes unnecessary words","A writer checks pronoun reference","A student improves sentence flow","A class checks subject–verb agreement","A writer chooses precise punctuation","A student corrects a run-on sentence","A class checks paragraph order","A writer varies sentence structure","A student checks capital letters","A class identifies a complete sentence","A writer improves cohesion","A student corrects an apostrophe","A class checks formal language","A writer replaces an ambiguous phrase","A student checks parallel structure","A class revises for meaning","A writer proofreads the final version","A student completes a final clarity check"];
  const Q=(year,skill,set,pos,type,question,extra={})=>{
    const contexts=skill==="reading-comprehension"?READING_CONTEXTS:skill==="vocabulary-word-meaning"?WORD_CONTEXTS:LANGUAGE_CONTEXTS;
    const item={
      id:`e-${year}-${skill}-s${set+1}-q${pos+1}`,
      year,subject:"english",skill,set,difficulty:LEVELS[pos],type,question:`${contexts[set%25]}. ${question}`,...extra
    };
