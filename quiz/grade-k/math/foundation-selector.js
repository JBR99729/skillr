/* ---------------------------------------------------------
   FOUNDATION PRACTICE SELECTOR
   ---------------------------------------------------------
   Main skill pages use short shuffled attempts from the available bank.
   --------------------------------------------------------- */

(() => {
  const config = window.exerciseConfig || {};
  const TARGET = Number(config.maxQuestions || 5);
  ...
  const selectedQuestions =
    shuffle(variants).slice(0, TARGET).map((q) => {
      const clean = { ...q };
