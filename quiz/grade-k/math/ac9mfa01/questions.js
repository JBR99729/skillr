...
(() => {
  ...
  const BUILDERS = {
    ...
    AC9MFA01: [
      ...
      (c) => { const p=rotate(PATTERN_SYMBOLS,c.v); const label=p.map((symbol,index,array)=>String.fromCharCode(65+array.indexOf(symbol))).join(""); return single(c,"match same rule",`Which letter rule matches this pattern?`,label,[`AAB`,`ABB`,`ABC`,`No repeating rule`],`Each symbol can be named with a letter. Match repeated symbols with the same letter.`,[...p,...p].join(" ")); },
...
window.SkillrFoundationRebuild.load("AC9MFA01");
